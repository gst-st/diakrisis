...
**Теорема 3.2** [Т·L3] (из 108.T пункт (4); полное доказательство — Следствие 5.10 в [`/12-actic/04-ac-oc-duality`](/12-actic/04-ac-oc-duality)). Для каждой артикуляции $\alpha$:

$$
\nu(\alpha) = \mathsf{e}\bigl(\varepsilon(\alpha)\bigr),
$$

где $\varepsilon(\alpha) \in \rangle\!\rangle \cdot \langle\!\langle$ — дуал-акт артикуляции $\alpha$ по 108.T, $\mathsf{e}(-)$ — ε-инвариант (активационная глубина акта). **ε- и ν-глубины совпадают** как ординалы. Доказательство опирается на Предложение 5.1 ($\varepsilon \circ \mathsf{M} \stackrel{\sim}{\Rightarrow} \mathsf{A} \circ \varepsilon$) и его трансфинитное расширение (Следствие 5.9).
...
Проверка `verum audit --epsilon` даёт ε-координату любого корпуса, параллельно $\nu$-координате. Это делает практики, а не только теоремы, полноправными гражданами системы.
