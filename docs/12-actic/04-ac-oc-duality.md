...
1. $\alpha \circ \varepsilon = \mathrm{id}_{\langle\!\langle \cdot \rangle\!\rangle}$ *строго*; $\varepsilon \circ \alpha$ gauge-эквивалентно $\mathrm{id}_{\rangle\!\rangle \cdot \langle\!\langle}$ через выбранный reflector $r$ в структуре объектов (MSFS Theorem~\ref{thm:ac-oc-duality}(b)).
...
Шаги A–F строго следуют MSFS §11 (Theorem~\ref{thm:ac-oc-duality} proof); ниже — изложение в Diakrisis-нотации с явными ссылками на технические источники.
