...
### 11.1 Полное доказательство 120.T 
...
Для целей 120.T используется MALL-версия (без full !), что достаточно для соответствия с $\mathrm{Perf}(\alpha_\mathrm{linear})$ как 2-категории. Расширение на full LL — отдельная задача через (T-2f*** для модальностей).
...
**Следствие 120.L3.1** *(связь с канонизацией).* Алгоритм $\mathrm{canonicalize}$ для gauge-equivalence (Конструкция 16.3 в /03-formal-architecture/16-gauge-decision) ограниченный на $\mathrm{Perf}(\alpha_\mathrm{linear})$ — это в точности cut-elimination в Ludics.
...
*Доказательство.* Жирар 2001 §1.2: $D_1 \perp D_2$ ⟺ их совместное выполнение приводит к divergence (no normal form). В $\mathrm{Perf}(\alpha_\mathrm{linear})$: divergence соответствует невозможности canonicalize обоих в одну gauge-орбиту (по Лемме 120.L3 + Лемма 16.2). ∎
...
Обе формализации — частные случаи $\mathrm{Perf}(\alpha_\mathrm{linear})$ через 120.T + (effect-system как Kleisli-категория, см. ниже).
...
### 12.1 Полное доказательство 121.T 
...
(syntactically), что даёт fibred product теорий. Через $\alpha$-функтор + 108.T (Предложение 5.1, теорема 124.T) сохранение продуктов:
...
(д) **$\phi = \exists/\forall x: D. A(x)$**: индексные продукты/копродукты по $\mathbf{U}_\mathrm{idx}$.

*Лемма (артикуляция кванторов)*. Для $\phi = \exists x: D. A(x)$:
$$\alpha_\phi \simeq \coprod_{x \in D \cap \mathbf{U}_\mathrm{idx}} \alpha_{A(x)} \quad \text{в } \langle\!\langle \cdot \rangle\!\rangle.$$

*Доказательство леммы*. Th(∃x.A(x)) = IPL + ∃x.A(x). По Skolemization (стандартно для accessible languages): доказательство ∃x.A(x) состоит из выбора witness $a \in D$ + доказательства $A(a)$. Это даёт teorija $\mathrm{Th}(\exists x. A(x)) \simeq \coprod_{a \in D} \mathrm{Th}(A(a))$ в категории теорий с морфизмами теории-расширений. Через α-функтор + 108.T сохранение копродуктов (108.T Шаг A полнота-верность сохраняет colimits): $\alpha_\phi \simeq \coprod_x \alpha_{A(x)}$.

Аналогично для $\forall$: $\mathrm{Th}(\forall x. A(x)) = \prod_x \mathrm{Th}(A(x))$ (parametric quantification).

Применяя 2-функториальность $\mathrm{Perf}$ к копродукту/продукту: $\mathrm{Perf}(\alpha_\phi) \simeq \coprod_x \mathrm{Perf}(\alpha_{A(x)})$ (или $\prod_x$ для $\forall$). По индукции $\mathrm{Perf}(\alpha_{A(x)}) \simeq \llbracket A(x) \rrbracket_\mathrm{BHK}$. Composition даёт $\mathrm{Perf}(\alpha_\phi) \simeq \coprod_x \llbracket A(x) \rrbracket_\mathrm{BHK} = \llbracket \exists x. A(x) \rrbracket_\mathrm{BHK}$ по Definition 121.D1.

Корректность бесконечных пределов — accessibility $\rangle\!\rangle\cdot\langle\!\langle$ + restriction на $\mathbf{U}_\mathrm{idx}$ (по Лемме 3.2 + стек-модели 131.T).
...
Утверждение двойное:
- **(OC-сторона)** $\mathsf{M} \dashv \widetilde{\mathsf{A}}$ как пара эндо-2-функторов $\langle\!\langle \cdot \rangle\!\rangle$.
- **(DC-сторона)** $\widetilde{\mathsf{M}} \dashv \mathsf{A}$ как пара эндо-2-функторов $\rangle\!\rangle \cdot \langle\!\langle$.

По 108.T эти утверждения эквивалентны.

### 14.2.2 Доказательство (OC-сторона): $\mathsf{M} \dashv \widetilde{\mathsf{A}}$
...
По Предложению 5.1: $\varepsilon \circ \mathsf{M} \simeq \mathsf{A} \circ \varepsilon$ как 2-естественная эквивалентность. По Предложению 6.2 (108.T): $\alpha \circ \varepsilon \simeq \mathrm{id}$ строго. Тогда для $\alpha_0, \beta \in \langle\!\langle \cdot \rangle\!\rangle$:
$$
\begin{aligned}
...
Второе тождество — симметрично, через те же рассуждения для пары $(\eta, \epsilon)$ относительно $\mathsf{M}$. ∎ (OC-сторона)

### 14.2.3 Доказательство (DC-сторона): $\widetilde{\mathsf{M}} \dashv \mathsf{A}$
...
**Эквивалентность с (OC-сторона)**: применением $\varepsilon_{*}$ к Φ (Шаг A) и $\alpha_{*}$ к Ψ (Шаг A'): обе цепочки дают эквивалентные структуры сопряжения. По 108.T (full faithfulness $\varepsilon, \alpha$) трансформации между сопряжениями переносятся точно.

Заключение: (DC-сторона) выполняется тогда и только тогда, когда (OC-сторона) — оба эквивалентны через 108.T. ∎ (DC-сторона)

### 14.2.4 Эквивалентность (OC-сторона) и (DC-сторона) через 108.T

По 108.T $\varepsilon \dashv \alpha$ — 2-эквивалентность, следовательно perspective change: любое сопряжение в $\langle\!\langle \cdot \rangle\!\rangle$ переносится в $\rangle\!\rangle \cdot \langle\!\langle$ через $\varepsilon$ и обратно. Это даёт каноническую эквивалентность утверждений (OC-сторона) и (DC-сторона).

### 14.2.4.A Связь между τ из Предложения 5.1 и unit/counit из 124.T

**Лемма 124.L_bridge** *(связь Предложения 5.1 и 124.T).* 2-естественная эквивалентность $\tau$ из Предложения 5.1 и unit/counit $(\eta, \epsilon)$ из настоящей теоремы 124.T связаны через сопряжение $\varepsilon \dashv \alpha$ (108.T) явной формулой:

$$
\eta_{\alpha_0} = \alpha\bigl(\tau_{\alpha_0}\bigr) \cdot (\alpha\circ\varepsilon)(\alpha_0)^{-1},
$$

где:
- $\tau_{\alpha_0}: \varepsilon(\mathsf{M}(\alpha_0)) \xrightarrow{\sim} \mathsf{A}(\varepsilon(\alpha_0))$ — компонента τ из Предложения 5.1.
- $\alpha(\tau_{\alpha_0}): \alpha\circ\varepsilon\circ\mathsf{M}(\alpha_0) \xrightarrow{\sim} \alpha\circ\mathsf{A}\circ\varepsilon(\alpha_0) = \widetilde{\mathsf{A}}\circ\mathsf{M}(\alpha_0)$.
- Композиция с $\alpha\circ\varepsilon \simeq \mathrm{id}$ даёт $\eta_{\alpha_0}: \alpha_0 \to \widetilde{\mathsf{A}}(\mathsf{M}(\alpha_0))$.

Аналогично для counit $\epsilon_\beta$: получается через $\varepsilon$-применение к compatibility τ + $\varepsilon\circ\alpha \simeq \mathrm{id}$.

*Доказательство Леммы.* Прямая проверка — обе стороны определены canonical способом из 108.T-данных + τ. Согласованность проверяется через triangle identities, которые работают как в Предложении 5.1, так и в 124.T. Это устанавливает, что **τ из Предложения 5.1 и unit/counit из 124.T — две стороны одного сопряжения**: τ — модификация, $(\eta, \epsilon)$ — её компоненты в специальном виде через 108.T-bridge. ∎
...
Сопряжение 124.T даёт операциональную дуальную семантику для Verum (operational coherence):
...
**Конструкция 140.K1**. В стек-модели $\mathfrak{M}^\mathrm{stack}_\mathrm{Diak}$ (теорема 131.T) для каждого $n \in \mathbb{N}$ имеется $(\infty, n)$-категория актов $\rangle\!\rangle\cdot\langle\!\langle^{(\infty, n)}$. Они связаны двумя направленными системами:
...
- (ii) Поглощения $\varepsilon$ на некотором ординале — обеспечено стек-моделью 131.T  с восхождением по универсумам $\kappa_1, \kappa_2$.
...
#### 15.3.7 Согласованность с стек-моделью 131.T 

**Лемма 140.L_stack** *(совместимость (∞,∞)-категории с стек-моделью).* Конструкция $\rangle\!\rangle\cdot\langle\!\langle_\infty$ из 140.K1 совместима с κ-башней стек-модели 131.T в следующем смысле:

- Каждый уровень $\rangle\!\rangle\cdot\langle\!\langle^{(\infty, n)}$ для $n \leq 2$ полностью помещается в $\mathbf{U}_2$-моделируемом универсуме (по Лемме 131.L_R + accessibility).
- Для $n \geq 3$ дальнейшие уровни — **не** в $\mathbf{U}_3$ (которого нет), а **через стабилизацию на $\mathbf{U}_2$** по Лемме 131.L3 (стек-стабилизация на объектном уровне через Drake reflection).
- Колимит $\rangle\!\rangle\cdot\langle\!\langle_\infty = \mathrm{colim}_n \rangle\!\rangle\cdot\langle\!\langle^{(\infty, n)}$ существует **формально** (как класс), но **не как объект** $\rangle\!\rangle\cdot\langle\!\langle$ (по Лемме 131.L2 — нерепрезентируемость колимита κ-башни).

*Заключение*: $\rangle\!\rangle\cdot\langle\!\langle_\infty$ — не один объект внутри стек-модели, а **трансфинитная башня объектов** с canonical embedding в стек-модель. ε-инвариант $\mathrm{e}^\infty$ корректно определён на каждом уровне и согласован между уровнями через truncation (Свойство 1).

#### 15.3.8 Заключение

Шаги 1–5 + Лемма 140.L_stack устанавливают существование $\mathrm{e}^\infty$ с заявленными свойствами, его универсальность и совместимость с κ-стратификацией стек-модели. ∎ 140.T
...
**Корректность в Eff.** Higher-order partial recursive functions с finite-stage realizability — стандартные объекты $\mathrm{Eff}$ через **modest sets** конструкцию: Hyland 1982 «The effective topos» §III «Modest sets» + Hyland-Ong-Robinson 1990 «The discrete objects in the effective topos» (классификация типов в Eff). $\omega^2$-iteration через diagonal nesting ω-уровней — каноническая конструкция в higher-type computability framework Eff (van Oosten 2008 «Realizability: An Introduction to its Categorical Side» §3 для расширенного изложения).
...
**Конструкция $\pi$ через стек-стабилизацию (Лемма 131.L3).** По Лемме 131.L3 (стек-стабилизация на объектном уровне): на $\mathbf{U}_2$-уровне дальнейшая мета-классификация имеет lift через стабилизирующее свойство — для $G \in \mathbf{U}_2$, $\mathsf{A}(G)$ интерпретируется как *внутренний рефлектор в $\mathbf{U}_2$ с тем же logical strength*.
