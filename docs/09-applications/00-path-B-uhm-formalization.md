...
Для теорем структурного ядра (TH-Final, 55.T–87.T, 98.T–99.T, 100.T–102.T) нормативным источником при Verum-формализации служит препринт **[*AFN-T*](/10-reference/04-afn-t-correspondence)** (`paper-en/paper.tex`): формулировки и доказательства в препринте — канонические, Diakrisis-документы ссылаются на них. При разработке Lean-формализации этих теорем используется preprint-нумерация (`thm:afnt-alpha`, `thm:five-axis`, и т. д.). Диакрисis-специфические конструкции (канонический примитив, 13 аксиом, gauge) формализуются отдельно.
