...
**[Т-набр]** — детальная формализация `/apeiron/03-extractions/02-zfc-extraction.md`.
...
- **Демонстрация Universal Foundation Theorem**: первый пример 29.T.
...
- **α_math**: Set или специфический object classifier.
...
| Foundation | well-foundedness ⊏_0 |
...
**Powerset** (∀A ∃𝒫(A)):
- В Diakrisis: внутренний хом [A, Ω], где Ω — subobject classifier.
...
## Grothendieck-universes = Fix-подмножества

**02.T3**: Grothendieck-universe — подмножество Fix(𝖬) в ZFC-ограничении.

### Формализация

**Grothendieck universe U** — множество, удовлетворяющее:
...
- Grothendieck universe существует ⟺ существует inaccessible cardinal.
- Inaccessible cardinal κ ↔ неподвижная точка 𝖬.
- ZFC + «существует inaccessible» ↔ Diakrisis с Fix(𝖬) ≠ ∅ в ZFC-ограничении.

## Морита с ETCS

По Lawvere, ZFC ↔ ETCS Морита-эквивалентны. В Diakrisis: α_ZFC ∼_{gauge} α_ETCS.
...
- **Морита-эквивалентна** ZFC (Shulman proof).
...
По 13.T, 𝖬(α_ZFC) содержит утверждения (G, Con(ZFC), инаксессибалы), не выразимые в ZFC. Это — **структурная** переупаковка Gödel.

### Детализация

**Gödel II (1931)**: Con(ZFC) не доказуема в ZFC (если ZFC консистентна).
...
Это — формальная переупаковка Gödel в языке Diakrisis.
...
- ZFC vs УГМ: существенная разница (ν_ZFC = ω, ν_uhm = ω·4).
