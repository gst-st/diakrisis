...
- Может быть полезна для анализа пределов (TH-Final — своего рода параконсистентное утверждение).
...
- **α_int → α_cl**: через double-negation translation.
- **α_cl → α_int**: не естественен (требует LEM).
- **α_lin → α_int**: через экспоненциал !.
- **α_cl → α_modal**: через translation ¬¬ = □.
...
- Пропозиции как типы.
- Кодирование логики в type theory.
