...
Diakrisis — **формальная реконструкция** этого ядра, насколько оно формально реконструируемо (по TH-Final — не полностью).
...
Diakrisis:
- Признаёт ограничение формализации (TH-Final).
...
## 12. Отношение с TH-Final

TH-Final говорит: Διάκρисίς **не формализуется** полностью.

Этот документ говорит: Διάκρисίς **дан в опыте**.

Оба — совместимы: Διάκρисίс — **не объект**, поэтому формальное описание его ограничено. Но как акт — он **не формальный**, и тем самым не подпадает под TH-Final (которая про формальные объекты).

Это — точно проведённое разграничение:

- **Формально** — Διάκρисίс не схватываем (TH-Final).
