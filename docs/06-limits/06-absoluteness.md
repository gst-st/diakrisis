---
sidebar_position: 6
title: Абсолютность AFN-T (пять осей)
---

# Абсолютность AFN-T — ABSOLUTA_TOTALIS
...
Независимость AFN-T от категорного уровня $n$. Прямое доказательство на уровне $(\infty, \infty)$ (MSFS Theorem `thm:direct-infty`, соответствует Diakrisis 66.T «$(\infty,\infty)$-Lawvere fixed-point») использует Lawvere FP в $(\infty, \infty)$-cartesian closed category $\mathcal{C}_S := \colim_n \mathrm{Syn}^{(\infty,n)}(S)$.

**Важное ограничение** (MSFS Remark `rem:direct-infty-scope`): прямое доказательство на $(\infty, \infty)$ работает только для **классических** $S$ (Boolean internal logic). Для конструктивных $S$ (HoTT, MLTT, CIC, CZF) используется основная Theorem `thm:afnt` через Lambek-Scott reflection.
...
ABSOLUTA_TOTALIS — это *комбинированное* утверждение: AFN-T + 5 осей = AFN-T (Absolute Foundation No-Go Theorem). Диаграмма зависимостей пяти осей в препринте — Figure `fig:five-axis`.

## Следующие теоремы

После установления ABSOLUTA_TOTALIS (пять осей) — три стандартных bypass-paths должны быть формально закрыты ([`/06-limits/08-intensional-refinement`](/06-limits/08-intensional-refinement), MSFS §8) и мета-классификация Level 5+ должна быть стабилизирована ([`/06-limits/09-meta-classification`](/06-limits/09-meta-classification), MSFS §9).

## Ссылки

- **MSFS §7** — основное изложение пяти осей;
- **MSFS §10** — positioning против Ernst 2015, Hamkins multiverse, BSP 2021;
