...
| **Полноты** | $\pi$ (условия Ловера) | 87.T | Theorem `thm:completeness` |
...
- **completeness subsumes horizontal**: каждая R-S лежит в условия Ловера $\mathcal{L}\mathcal{S}$ по (R5).
...
### Полноты — 87.T (условия Ловера)

Полная характеризация: любая foundational-theoretic вариация сводится к одной из четырёх осей в пределах условия Ловера $\mathcal{L}\mathcal{S}$. Пятая ось $\pi$ закрывает вопрос «есть ли незамеченная ось?» формально.
...
После установления пятиосевая абсолютность AFN-T (пять осей) — три стандартных обходных пути должны быть формально закрыты ([`/06-limits/08-intensional-refinement`](/06-limits/08-intensional-refinement), MSFS §8) и мета-классификация уровень 5+ должна быть стабилизирована ([`/06-limits/09-meta-classification`](/06-limits/09-meta-classification), MSFS §9).
