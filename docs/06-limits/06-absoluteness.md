...
Независимость AFN-T от категорного уровня $n$. Прямое доказательство на уровне $(\infty, \infty)$ (препринт Theorem `thm:direct-infty`, соответствует Diakrisis 66.T «$(\infty,\infty)$-Lawvere fixed-point») использует Lawvere FP в $(\infty, \infty)$-cartesian closed category $\mathcal{C}_S := \colim_n \mathrm{Syn}^{(\infty,n)}(S)$.
