...
по компоненциональности внутренней метатеории (R3). Следовательно $\mathsf{M}_S$ сохраняет $\aleph_1$-filtered colimits $\aleph_1$-presentable объектов (Adámek–Rosický 1994, Thm 2.26). ∎
...
- Аналог в MSFS: Definition `def:strcat` + Lemma `lem:SS-membership` обеспечивают то же вложение, но через $\mathbf{StrCat}_{S, n}$, не через $\langle\!\langle \cdot \rangle\!\rangle$. Эквивалентность двух конструкций — стандартная (через Yoneda).
...
**105.T** [Т·L3]. *Аксиома T-2f\* депть-стратификации блокирует любой самореферентный парадокс, сводимый к универсальной диагональной конструкции Яновского (Yanofsky 2003). В частности: для любой артикуляции $\alpha \in \langle\!\langle \cdot \rangle\!\rangle$ удовлетворяющей T-2f\*, не существует формулы $\phi(x)$ в $L_\alpha$ с самореферентным противоречием.*

Следствие: все семейства самореферентных парадоксов (Russell, Cantor, Burali-Forti, Tarski, Lawvere, Girard, Gödel-type диагонали, Grelling-Nelson, Curry, и любое *другое* диагональное построение) заблокированы.

### Предварительные результаты

**Теорема Яновского (Yanofsky 2003)**. Все известные самореферентные парадоксы (Cantor, Russell, Burali-Forti, Gödel, Tarski, Lawvere, Löb) — специализации единой категорной конструкции:
...
- Cantor: $Y = X$, $T = \{0,1\}$, $\alpha(x) = [\chi_x]$. $f = \neg$ → ∄ биекция $X \leftrightarrow \mathcal{P}(X)$.
- Tarski: $Y = T = \mathrm{Sent}(L)$, $\alpha =$ Gödel-numbering, $f = \neg$ → ∄ truth predicate.
- Lawvere: та же схема в произвольном ccc.
- Gödel: $f = $ «$\mathrm{Provable}$», не имеет неподвижной точки → ∄ consistent complete расширение.
...
**Шаг 3. Depth-блокировка Яновского-$\alpha$.** Рассмотрим Yanofsky-морфизм $\alpha: Y \to T^Y$ в $\alpha \in \langle\!\langle \cdot \rangle\!\rangle$. По Шагу 2:
...
**Шаг 4. Блокировка любого парадокса.** По Yanofsky 2003, каждый самореферентный парадокс сводится к существованию w.p.s. морфизма $\alpha: Y \to T^Y$ с $f: T \to T$ без неподвижной точки. Шаг 3 исключает w.p.s. в T-2f\*-стратифицированных артикуляциях. Следовательно любой такой парадокс заблокирован. ∎
...
**105.C2** *(Расширение 18.T)*. Теорема 18.T перечисляла 5 семейств парадоксов (Russell, Curry, Grelling, Burali-Forti, Girard); 105.T обобщает её до **универсальной** иммунности через Yanofsky-сводимость.

**105.C3** *(Сравнение с ramified type theory)*. T-2f\* — это $(\infty, \infty)$-категорная версия ramified type hierarchy Russell–Whitehead. В отличие от original PM, T-2f\* автоматизируется через $\mathsf{M}$-итерации, не требуя явной типизации.

### Замечания к 105.T

- Ключ — Yanofsky 2003. Без универсальности Яновского мы можем блокировать только *известные* парадоксы, не всех. Yanofsky даёт универсальность: «всё self-reference — диагональная конструкция».
- Возможная критика: могут существовать самореферентные парадоксы, *не* сводимые к Yanofsky. Ответ: Yanofsky 2003 + последующие работы (Lawvere 1969, Roberts 2023 "A simple proof of Yanofsky's universal theorem") показывают, что любой парадокс диагонального характера в cartesian-closed категории сводится к Yanofsky. Остаются только парадоксы, использующие не-ccc структуру (например, парадоксы in quantum logic), которые *не* относятся к самореференции в нашем смысле.
...
**(Max-3)** Depth-стратификация. Прямое следствие 105.T: T-2f\* блокирует все Yanofsky-сводимые парадоксы универсально. ∎
...
3. **Неконструктивные парадоксы.** 105.T блокирует все Yanofsky-сводимые парадоксы. Существуют ли парадоксы, не сводимые к универсальной диагональной конструкции? Yanofsky 2003 + расширения (Roberts 2023) утверждают универсальность; но полная формализация этой универсальности для $(\infty, \infty)$-категорной семантики — отдельная работа.
