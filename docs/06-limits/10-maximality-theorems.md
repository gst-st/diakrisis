...
**[Т]** Для (Max-4) — MSFS Theorem `thm:срез-локальность` (= 99.T).
...
Формально: существует 2-функтор $\mathrm{Artic}: \mathcal{F} \to \langle\!\langle \cdot \rangle\!\rangle$, компонируемый с $\mathrm{Cl}_\mathrm{Diakrisis}$ в канонический калибровочный фактор $\mathcal{F} \to \mathfrak{M}_\mathrm{Fnd}$. Следствие:
...
**Шаг 1. Синтаксическая $(\infty, n_S)$-категория.** Для любой $S \in \mathrm{R\text{-}S}$ определяем $\mathrm{Syn}(S)$ — синтаксическую $(\infty, n_S)$-категорию (конструкция Линденбаума–Тарского, адаптированная к зависимым типовым теориям: Сили 1984, Хофман 1997, Капулкин–Ламсдейн 2021 для $\HoTT$, Люри HTT §6 для $\infty$-случая). Существование $\mathrm{Syn}(S)$ гарантировано условием (R1) (интерпретация $\mathsf{Q}$) + (R2) (рекурсивная перечислимость) + (R5a) (доступная $(\infty, n_S)$-категорная семантика).
...
где $\mathrm{Code}_S(X)$ — внутренний код $X$ как объекта, существующий по (R4) гёделевому кодированию. На морфизмах $f: X \to Y$ действует как $\mathrm{Code}_S(f): \mathrm{Code}_S(X) \to \mathrm{Code}_S(Y)$.

**Лемма 103.L1** (*Accessibility $\mathsf{M}_S$*). $\mathsf{M}_S$ — $\aleph_1$-accessible эндофунктор.

*Доказательство*. По (R2), аксиомы $S$ — рекурсивно перечислимы, следовательно формулы языка $L_S$ — не более чем счётного количества. $\mathrm{Code}_S(X)$ для $X \in \mathrm{Syn}(S)$ представим формулой $\phi_X$, причём $\lvert \mathrm{sub}(\phi_X) \rvert < \aleph_0$ (конечное число подформул). Для $\aleph_1$-фильтрованного копредела $X = \colim_{i} X_i$ имеем:

$$
\mathrm{Code}_S(\colim X_i) = \colim \mathrm{Code}_S(X_i)
$$

по покомпонентности гёделева кодирования (R4) и сохранению $\aleph_1$-фильтрованных копределов в $(\infty, n_S)$-категории $\mathrm{Syn}(S)$ по (R5a). Следовательно $\mathsf{M}_S$ сохраняет $\aleph_1$-фильтрованные копределы $\aleph_1$-предъявимых объектов (Адамек–Росицкий 1994, теорема 2.26). ∎
...
3. $\mathrm{Syn}(S)$ замкнута под нужными копределами ✓ (по (R5a) $\kappa_S$-доступности $(\infty, n_S)$-категории).
...
- (R5b) сопряжение Ламбека–Скотта $(\mathrm{Syn}, \mathrm{Mod})$ используется в Шаге 4 для корректности $\alpha_f$ как 2-морфизма: естественность кодирования под интерпретациями обеспечивается единицей сопряжения, которая по (R5b) является $(\infty, n_S)$-эквивалентностью на начальный объект.
...
То есть **каждая Морита-эквивалентность между Rich-основаниями реализуется как калибровочное преобразование в Diakrisis**.
...
На остальном $\langle\!\langle \cdot \rangle\!\rangle$ (не-R-S артикуляции — субструктурные, лимитные): расширяем $\tilde\sigma$ как тождество. Это даёт корректно определённый 2-функтор, поскольку не-R-S артикуляции либо не связаны морфизмами с R-S (случай $\alpha_\mathrm{affine}$, $\nu \leq \omega$ — см. 54.T), либо связаны через функториальность (композиция, пределы), которые $\tilde\sigma$ сохраняет по индукции.
...
где равенство $\alpha_{S, \mathsf{M}} = \alpha_{\sigma(S), \mathsf{M}_{\sigma(S)}}$ по естественности гёделева кодирования (R4) под 2-автоэквивалентностями. ∎
...
- Ключ — Яновский 2003. Без универсальности Яновского мы можем блокировать только *известные* парадоксы, не всех. Яновский даёт универсальность: «всякая самореференция — диагональная конструкция».

#### Точная формулировка области применимости (Яновский-сводимость)

**105.T строго формулируется внутри Яновский-сводимой области:**

**Определение (Яновский-сводимый парадокс).** Самореферентный парадокс $\Pi$ называется *Яновский-сводимым*, если его категорное ядро редуцируется к диагональной конструкции $\alpha: Y \to T^Y$ вместе с эндоморфизмом $f: T \to T$ без неподвижной точки в декартово-замкнутом или замкнуто-симметрическо-моноидальном контексте, где $Y$ и $T^Y$ допускают сравнимое индексирование глубины (Яновский 2003).

**Яновский-сводимы:** Рассел, Кантор, Burali-Forti, Тарский, Ловер, Жирар $\mathrm{Type}: \mathrm{Type}$, Gödel-type диагонали, Curry, Grelling-Nelson — все классические семейства самореференции через диагональ-на-экспоненциалах.

**Не покрыты автоматически 105.T:**
- *Парадоксы Berry-типа*: оперируют предикатом определимости в объемлющей метатеории, а не категорной диагональю на экспоненциалах. Зависят от метатеоретического определимого предиката, не от Яновский-структуры в самой $\alpha$.
- *Модальные парадоксы Лёба*: оперируют модальным оператором доказуемости, не диагональю на экспоненциалах. Структурно выходят за рамки Яновский 2003.
- *Парадоксы в принципиально неклассических объемлющих логиках* (например, паранепротиворечивых метатеориях с диалетея-валидным отрицанием), где Яновский-конструкция перестаёт быть универсальной.

**105.T корректно гарантирует:** иммунность от всех Яновский-сводимых парадоксов. Является ли *каждый* возможный самореферентный парадокс в $\mathrm{R\text{-}S}$-основаниях Яновский-сводимым — открытая мета-теорема. Соответственно: 106.T (Diakrisis $\in \mathcal{L}_{\mathrm{Cls}}^{\top}$) — теорема **относительно области Яновский-сводимости**: любой гипотетический не-Яновский-сводимый парадокс, допускаемый артикуляцией, лежал бы вне гарантии 105.T и потребовал бы усиленного условия (T-2f\*\*\*) с дополнительным блокированием на уровне внутреннего языка.

**Согласованность с MSFS:** соответствует scope-формулировке MSFS Theorem `thm:meta-cat` (мета-категоричность $\mathcal{L}_{\mathrm{Cls}}^{\top}$ относительно области Яновский-сводимости).
...
**106.C4** *(Стабильность под мета-классификацией)*. 102.T + 106.T: итерированная мета-классификация Diakrisis воспроизводит ту же $(\infty, \infty)$-теорию (на теоретическом уровне стабилизация), с восхождение по универсумам по $\kappa_1 < \kappa_2 < \ldots$.
...
- **Интенсиональная полнота** — Theorem `thm:срез-локальность` (= 99.T).
...
| (Max-2) калибровочная полнота | [Г] | **[Т]** (104.T) |
...
## Теорема 128.T — Структура ядра калибровочной сюръекции
...
$$K \;\simeq_2\; \mathrm{Aut}_2^{\mathrm{калибр\text{-}тожд}}\bigl(\langle\!\langle \cdot \rangle\!\rangle_{\mathrm{nR\text{-}S}}\bigr),$$
...
**Шаг 1** (характеризация $K$). $\tilde\sigma \in K$ ⟺ для каждого $S \in \mathrm{R\text{-}S}$: $\tilde\sigma(\alpha_S) \sim_\mathrm{калибр} \alpha_S$. По 103.T-Шаг 4 (естественность артикуляции) + единственности единицы Ламбека–Скотта (R5b), $\tilde\sigma|_{\langle\!\langle \cdot \rangle\!\rangle_{\mathrm{R\text{-}S}}}$ — тождество с точностью до канонического калибровочного 2-изо.

**Шаг 2** (расщепление в калибровочном факторе). $\mathrm{калибр}\text{-}\mathrm{фактор}(\langle\!\langle \cdot \rangle\!\rangle) = \mathrm{калибр}\text{-}\mathrm{фактор}(\langle\!\langle \cdot \rangle\!\rangle_{\mathrm{R\text{-}S}}) \oplus \mathrm{калибр}\text{-}\mathrm{фактор}(\langle\!\langle \cdot \rangle\!\rangle_{\mathrm{nR\text{-}S}})$ — биколокализованная сумма *на уровне калибровочного фактора*; морфизмы между R-S и не-R-S артикуляциями (например, ГГК-вложение) при калибровочной факторизации схлопываются по 54.T. Обе под-2-категории сито-стабильны под $\mathsf{M}$ (Axi-2 + 54.T).

**Шаг 3** ($K$ как ограничение на $\mathrm{nR\text{-}S}$). Если $\tilde\sigma \in K$, то по Шагу 1 $\tilde\sigma|_{\mathrm{R\text{-}S}} = \mathrm{id}$ канонически. Следовательно, $\tilde\sigma$ полностью определена на $\langle\!\langle \cdot \rangle\!\rangle_{\mathrm{nR\text{-}S}}$ с условием тождественности на калибровке: $K \simeq_2 \mathrm{Aut}_2^{\mathrm{калибр\text{-}тожд}}(\langle\!\langle \cdot \rangle\!\rangle_{\mathrm{nR\text{-}S}})$.
...
- **128.C2**. *Конкретные элементы $K$*: линейные сдвиги в субструктурном фрагменте (54.T: $\alpha_\mathrm{linear}, \alpha_\mathrm{affine}, \alpha_\mathrm{relevant}$ — не R-S по (R5b)-провалу для не-классических связок); лимитные перестановки на $\mathrm{lim}_\kappa \alpha_\kappa$; паранепротиворечивые сдвиги (LP, Рутли–Мейер). ГГК-сопряжение $\mathfrak{g}_\mathrm{logic}$ — **не** в $K$ (действует между R-S артикуляциями).

## Теорема 129.T — Инициальность Diakrisis в $\mathcal{L}_\mathrm{Cls}^\top$

### Формулировка

**129.T** [Т·L3]. *Diakrisis — инициальный объект в $\mathcal{L}_\mathrm{Cls}^\top$ над $\mathrm{ZFC} + 2\text{-inacc}$. Для каждого $\mathbf{F} \in \mathcal{L}_\mathrm{Cls}^\top$ над той же R-S существует $\Phi_\mathbf{F}: \mathrm{Diakrisis} \to \mathbf{F}$, единственный с точностью до канонического $(\infty, 2)$-изо, такой что $\mathrm{Cl}_\mathbf{F} \circ \Phi_\mathbf{F} \simeq \mathrm{Cl}_\mathrm{Diakrisis}$ и $\mathbb{I}_\mathbf{F} \circ \Phi_\mathbf{F}^\mathrm{op} \simeq \mathbb{I}_\mathrm{Diakrisis}$; $\Phi_\mathbf{F}$ — $(\infty, \infty)$-эквивалентность.* Растворяет вопрос «второго представителя» через каноническую жёсткость.

### Лемма 129.L1 (совместная точность $(\mathrm{Cl}, \mathbb{I})$ на $\mathcal{L}_\mathrm{Cls}^\top$)
...
*Доказательство*. По (Max-4)→(I-4) Морита-как-2-локализация, $\mathcal{U} \circ \mathbb{I}_\mathbf{F} \simeq \mathrm{Cl}_\mathbf{F}$ (MSFS Theorem `thm:I-existence` Шаг 8). По (I-3) строгое уточнение Морита, $\mathbb{I}_\mathbf{F}$ различает Морита-эквивалентные пары: проекция несёт информацию строго точнее калибровочного класса. Морфизм определён парой $(\mathrm{Cl}, \mathbb{I})$ однозначно. ∎
...
**Шаг 2** (поднятие в $\mathbf{F}$). По (Max-1), $\mathrm{Cl}_\mathbf{F}$ существенно сюръективен на $\mathfrak{M}_\mathrm{Fnd}$. Универсальное свойство Гротендиковой выпрямки (Люри HTT 3.2.0.1) к диаграмме над $\mathfrak{M}_\mathrm{Fnd}$ даёт единственное (с точностью до канонического $(\infty, 2)$-изо) поднятие $\Phi_\mathbf{F}: \mathrm{Diakrisis} \to \mathbf{F}$.

**Шаг 3** (совместимость с $\mathbb{I}$). По Лемме 129.L1, $\Phi_\mathbf{F}$ автоматически согласован с интенсиональной стороной через срез-проекцию (MSFS Theorem `thm:срез-локальность`).
...
- **129.C2** *(Закрытие MSFS Q1)*. Q1 положительно отвечен 106.T (свидетель существует), 129.T дополнительно даёт каноническую жёсткость (свидетель уникален как инициальный объект).
...
- **Категорически единственная** среди максимальных классификаторов: каноническая инициальность (129.T).
- **Универсально парадокс-иммунная**: T-2f\* (105.T) + T-2f\*\* модальная стратификация (см. [`/02-canonical-primitive/02-axiomatics`](/02-canonical-primitive/02-axiomatics) §T-2f\*\*) для Berry / Лёба / паранепротиворечивых.
- **Срез-локальная интенсионально** (99.T).
- **Калибровочно-полная** относительно Морита-эквивалентностей R-S (104.T) с явной структурой ядра (128.T).
- **Классификационно-полная** относительно $\mathfrak{M}_\mathrm{Fnd}$ (103.T).

Программная составляющая закрыта: 128.T, 129.T выше + T-2f\*\* и реализация Axi-8 в стек-модели в [`/02-canonical-primitive/02-axiomatics`](/02-canonical-primitive/02-axiomatics).
