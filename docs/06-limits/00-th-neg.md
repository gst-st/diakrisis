...
**[Т-набр]** — доказано структурно. Отчёт в `/apeiron/seed/05-negative-result.md`.
...
- Т.е. M_F ∈ 𝒮.
- **Источник**: определение мат-модели в ZFC.
- **Следствие**: объекты M_F — элементы стандартных структур.

:::note Уточнение после audit2 (2026-04-22)

Лемма 2 в исходной форме была формально неточной: класс 𝒮 не был явно определён.
После audit2 уточнено:

- 𝒮 = 𝒮_S^{local} ∪ 𝒮_S^{global}, где:
  - 𝒮_S^{local} = объекты в Ob(M_F).
  - 𝒮_S^{global} = natural transformations, sections, derived constructions.

- **Лемма 2''** (локальная): Ob(M_F) ⊆ 𝒮_S^{local}.
- **Лемма 2'''** (глобальная): (F_S)(X) ⇒ X ∈ 𝒮_S через categorical semantics (56.T).

Детали: [/06-limits/06-absoluteness](/06-limits/06-absoluteness).

Это уточнение **сохраняет** TH-Final и закрывает universe-polymorphism-based возражения.
