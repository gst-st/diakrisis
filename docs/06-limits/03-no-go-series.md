...
Классически перечисляются пять (Cantor → Tarski). AFN-T добавляет шестую по причинам:
...
С Dual-AFN-T классическая серия закрывается симметрично: Cantor → Russell → Gödel → Tarski → Lawvere → AFN-T (ОЦ) ↔ Dual-AFN-T (ДЦ). Подробнее — [`/12-actic/05-dual-afn-t`](/12-actic/05-dual-afn-t).
