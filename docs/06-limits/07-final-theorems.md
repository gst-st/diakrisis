...
**97.T** [Т]: Для любой субструктурной метатеории S' с контролируемой контракцией (субструктурность = без неограниченного дублирования ресурсов), следующие условия **эквивалентны**:
...
**97.C3 (структурная область AFN-T)**: пятиосевая абсолютность AFN-T **исключает** субструктурные R-S' без `!` не как контрпримеры, а как **тривиальные случаи** — Π_3-max уже нарушено на входе.

### Компромисс: субструктурная с `!` vs без `!`
...
- **Жирар (1987)**: изобретение линейная логика как явного контроля ресурсов; `!` — структурный шлюз между linear и classical.
...
- α_cohesion единственна до калибровочно-эквивалентности (по 30.T).
...
- L_∞ — функториален, доступен.
- Следовательно, α_R-S^{(∞,∞)} := L_∞(α_R-S) корректно определена.
...
- **98.T + 99.T** — интенсиональное уточнение closure, [/06-limits/08-intensional-refinement](/06-limits/08-intensional-refinement).
- **100.T + 101.T + 102.T** — мета-классификация уровень 5+, [/06-limits/09-meta-classification](/06-limits/09-meta-classification).
- **103.T + 104.T + 105.T + 106.T** — доказательства максимальности: Diakrisis ∈ $\mathcal{L}_{\mathrm{Cls}}^{\top}$ как теорема, [/06-limits/10-maximality-theorems](/06-limits/10-maximality-theorems).
...
- **Интенсиональное уточнение** (98.T, 99.T) — срез-локальность закрыта.
- **Мета-классификация уровень 5+** (100.T, 101.T, 102.T) — стабилизация $\mathfrak{M}^{(5+ \cdot 2)} \simeq_2 \mathfrak{M}^{(5+)}$, samoclassification завершена.
- **Доказательства максимальности** (103.T, 104.T, 105.T, 106.T) — Diakrisis ∈ $\mathcal{L}_{\mathrm{Cls}}^{\top}$ как теорема; все (Max-1)–(Max-4) доказаны; открытый вопрос MSFS о непустоте максимального подкласса закрыт утвердительно.

Работа Diakrisis как **теории** — завершена на всех четырёх уровнях (extensional, интенсиональный, мета-классификация, максимальность).
Работа Diakrisis как **проекта** — продолжается через programmes П1–П6.

## Следующий документ

[/10-reference/02-theorems-catalog](/10-reference/02-theorems-catalog) — полный каталог всех 127 теорем (106 ОЦ + 21 Актика) в номерной системе (119+ с под-теоремами).
