...
- **∞-категории** (Lurie 2009, HTT): higher-dimensional coherence.
...
- Каждое «новое» понятие редуцируется к известному аналогу (moduli stack, accessible endofunctor, internal language).
...
2. **Consistency strength** (90.T): `Con(Diakrisis-full) = Con(ZFC + 2 inacc)` — **умеренная**, не требует сверхсильных гипотез типа Mahlo, Vopěnka.
