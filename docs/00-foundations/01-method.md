...
Это — прямое следствие опыта framework-фазы (88 документов, ~160 теорем), где **без** атакующей проверки была эскалация риторики до ложной претензии на уровень 6.

### 0.1 Эпистемический фон

Метод возник как ответ на конкретную проблему: **риторическая инфляция без контроля**. В фазе framework система накапливала заявления о новизне, не подвергая их формальной проверке:

- «Классифицирующее пространство всех оснований» — заявлено, но редуцируется к стандартной moduli stack.
- «Самопорождающий акт 𝖠» — заявлен, но совпадает с accessible endofunctor.
- «Escape Theorem» — заявлена как новая, но является переупаковкой Gödel II.

Каждое такое заявление в отдельности **не было** ошибкой; ошибкой была **систематическая** эскалация метаязыка без атакующей проверки. Метод Diakrisis закрывает эту лазейку.
...
**Подэтапы**: в этом этапе допускается **многократная итерация** (в framework было 46 итераций 01-primitive; в Diakrisis — финальная форма).
...
**Критерий выхода**: (а) каждое структурное свойство доказано или явно помечено как [Т-набр]; (б) соответствие с известными аналогами (Shulman 2-topos, Schreiber cohesion, Lurie ∞-topos) проверено; (в) gauge-группа явно описана.
...
**Результат**: TH-Neg, TH-Neg-extended, TH-Final. Признание, что предельное основание уровня 6 невозможно.

**Критерий выхода**: каждая no-go теорема (а) имеет полное формальное доказательство или контур с явной пометкой; (б) размещена в контексте no-go-традиции (Cantor..Lawvere..TH-Final).

### Этап 6 — Следующий цикл (итерация)

После прохождения 0..5 открывается **новый цикл** с поправками. Этап 6 — это возврат к Этапу 0 с обновлёнными правилами.

**Пример**: seed-фаза (в нашей истории) — это Этап 6 относительно framework-фазы.
...
Этот цикл применялся в seed-фазе ко всему framework-корпусу. Результат — TH-Final (множество заявленных новизн редуцировались).
...
**Примеры прошлых редукций (в seed-фазе)**:

| X (framework) | Редукция | Известный автор |
|---|---|---|
| 𝖠 | accessible endofunctor | Adámek-Rosický |
...
**Пример**: УГМ-сборка генеративна (даёт конкретные предсказания: P_crit=2/7, Φ_th=1, и т.д.). Framework как целое — генеративен частично (некоторые теоремы о 𝓜_Fnd — новые).
...
## Различие двух режимов работы

### Режим framework (ошибочный)

- Additive: накапливать структуры, теоремы, связи.
- Рhetoric-friendly: допускаются слова «нерушимый фундамент», «сердце Логоса», «восемь закрытий».
- Без строгой проверки новизны.
- Результат: богатый катаклог, но с завышенной метадатой (уровень 6 вместо 5).

### Режим Diakrisis (строгий)

- Attack-driven: каждая структура проверяется на редукцию.
- Rhetoric-avoidant: запрещены эскалирующие слова.
- Признание провала — валидный результат.
- Результат: меньше структур, но с точной метадатой.

Переход от framework-режима к Diakrisis-режиму произошёл через seed-фазу.

### 3.1 Детальное сопоставление режимов

| Параметр | Framework-режим | Diakrisis-режим |
|---|---|---|
...
### 3.2 Предупреждающие сигналы возврата в framework-режим
...
**Практическое следствие**: в /apeiron/ было 46 итераций одного документа; в Diakrisis — финальная форма. Если появляется 47-я итерация, это сигнал к структурному пересмотру.
...
**Практическое следствие**: переход framework → Diakrisis **не** означал удаление ~160 теорем. Означал переинтерпретацию их метаданных.
...
- **Ссылки** на связанные документы и на framework/seed-корпус.
...
### 7.2 Исторические примеры нарушений

- **04-assemblies/uhm.md (framework)**: N1 (метаязык «предельное основание»), N4 (редукция к NCG не документирована).
- **06-recognitions/...md (framework)**: N1, N3.
- **01-primitive iterations 42-46**: S-3 нарушение (цикл без осознания).

Каждое нарушение — не катастрофа, а повод для ревизии. В seed-фазе таких ревизий было ~30.
...
- **Formal verification (Coq, Lean)** — машинная проверка корректности.
