...
По Axi-8 (в строгой форме), α_𝖬 **не** Yoneda-представим. Но в Cat-модели α_𝖬 оказывается Yoneda-представим (поскольку ι в Cat — стандартное вложение). Это — один из аспектов, обсуждаемых в TH-Final.
...
**Контур доказательства**: по теореме Adámek-Rosický, accessible endofunctor на locally presentable category имеет инициальную алгебру, которая является неподвижной точкой 𝖬.
...
**Замечание**: в framework-корпусе Trace(𝖠) часто использовался как первичный объект. В Diakrisis — **производный** объект (П-0.2 экономия).
...
По Adámek (1974): если 𝖬 accessible, то существует начальная 𝖬-алгебра A_init = colim_{κ<λ} 𝖬^κ(⊥), где λ — регулярный кардинал больший rank(𝖬), ⊥ — инициальный объект.
