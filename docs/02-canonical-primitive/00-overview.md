...
:::info 2-Cat — выбор, не необходимость (после audit3, 2026-04-22)

Исходная Diakrisis выбрала 2-категорию как pragmatic choice (минимальность + формализуемость в prover'ах). Это **не** универсальная база: Diakrisis может быть расширена до (∞,n)-category-based версии для любого n, включая ∞.

**Следствие audit3**: параллельно с исходной 2-Diakrisis существует **∞-Diakrisis** — расширение с (∞,∞)-семантикой. В ней ⟪⟫^{∞} имеет нетривиальные k-морфизмы для всех k.

**Связь**: 2-Diakrisis = τ_{≤2}(∞-Diakrisis) через 2-truncation.

**TH-Final**: сохраняется в обоих контекстах (55.T для 2-version, 59.T для (∞,∞)).
