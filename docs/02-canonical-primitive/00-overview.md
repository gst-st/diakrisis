...
Но: все эти функции **не выходят** за пределы стандартной мат-техники (2-категории, доступность, модули-стеки). «+» — это **комбинация** и **применение**, не новый тип математики.
...
- **𝖬 — доступный**: существует регулярный кардинал $\lambda$ такой, что 𝖬 коммутирует с $\lambda$-фильтрованными копределами.
...
- Для достаточного λ: 𝖬^λ(α) стабилизируется (по доступности, теорема Адамека–Росицкого).
...
- **Axi-8 (M-5w\*)** — нетривиальность: α_𝖬 не Ёнеда-представим (критерий **потенциальной** новизны; в каноне признано, что в Cat-модели этот критерий нарушается, но в не-LP моделях может выполняться — см. [/03-formal-architecture/00-metacategory-structure](/03-formal-architecture/00-metacategory-structure)).
- **Axi-9** — достаточность ⟪⟫.
...
- **10.T5** — существование Ω̄ при доступность 𝖬.
...
- **43.T1** — Классифицирующее пространство: Trace(𝖠)/gauge ≃ 𝓜_Fnd (moduli-пространство оснований).
...
- **Комбинация** специфических черт (когезия + калибровка + внутренняя замкнутость + модальность).
...
Примеры калибровочно-эквивалентности:
- ZFC и ETCS — калибровочно-эквивалентны (Морита).
- HoTT с univalence и без — **не** калибровочно-эквивалентны (univalence — существенная часть).
- УГМ с разными выборами инвариантов — калибровочно-эквивалентны, если инварианты функционально эквивалентны.
