...
α_𝖬 **не** Yoneda-представим. (В Cat-модели 10.T1 нарушено; цель — найти не-LP модели.)

#### Обоснование

- Если α_𝖬 был бы Yoneda-представим, он сводился бы к стандартному представимому функтору, и вся специфика Diakrisis редуцировалась бы.
- Axi-8 — **потенциальный** критерий новизны: если удовлетворён, то α_𝖬 — «активный» (не-пассивный) функтор.
- В стандартной Cat-модели **нарушается** (это — часть TH-Final).

#### Признание ограничения

- В Cat-модели 10.T1: α_𝖬 = ι(𝖬) и в Cat, ι(𝖬) — Yoneda-представим через конкретный объект. Axi-8 **нарушается**.
- Попытки обойти: искать не-LP модели, где Axi-8 выполняется. Открытая задача.
- По TH-Final: полное удовлетворение Axi-8 + accessibility 𝖬 + прочие условия — **невозможно**.
...
- Это — **структурное** ограничение, не постулат существования конкретной γ.
...
Russell-иммунитет (Теорема 10.T2).

#### Обоснование

- Без T-2f\* наивная комплетация {α : P(α)} могла бы приводить к Russell-подобным парадоксам.
...
**Теорема 21.T2** (из `/apeiron/01-primitive/21-consistency-and-independence.md`): каждая аксиома Axi-0..Axi-9 + T-α + T-2f\* **независима** (существует модель, удовлетворяющая всем остальным, но не выделенной).
...
| Axi-7 | Без α_𝖬 |
| Axi-8 | Все α_𝖬 Yoneda-представимы |
...
В этой модели **выполняются** Axi-0..Axi-7, Axi-9, T-α, T-2f\*. **Axi-8 (M-5w\*)** — нарушается, так как ι(𝖬) в Cat — Yoneda-представим.
...
- **∞-категорные модели**: работа в Lurie HTT.

Ни в одной альтернативной модели **всех** 13 аксиом одновременно удовлетворить (в сильной форме) не удалось — что согласуется с TH-Final.
...
## Эволюция аксиом (исторический контекст)

### Framework-фаза

В `/apeiron/01-primitive/` было ~20+ первоначальных аксиом:
- Ax-0, Ax-S, Ax-L (три формы для разных моделей).
- Ax-R (рефлексивность 𝖠).
- Ax-1, Ax-2, Ax-3, Ax-4, Ax-5 (исходный набор).
- И дополнительные.

### Seed-фаза

- Ax-R доказан как теорема — **удалён** из аксиом.
- Ax-3 (самоартикуляция) заменена теоремой 10.T3.
- Ax-5 (неполнота) заменена теоремой 10.T4.
- Формализация через четвёрку (⟪⟫, 𝖬, α_math, ⊏_•) + 13 условий.

### Diakrisis (финальная форма)

- 13 условий: Axi-0..Axi-9 + T-α + T-2f\*.
- Каждое проверено на независимость.
- Вся система консистентна (10.T1).
