...
*Доказательство*. **Шаг 1** (omega-модальный случай). Парадокс $A \leftrightarrow \bigwedge_{n < \omega} \Box^n A$ (бесконечная модальная конъюнкция самоприменения) требует $\mathrm{md}^\omega(A) = \omega$. По T-2f\*\*\* нужно $\mathrm{md}^\omega(A) > \omega$, противоречие. ∎

**Шаг 2** (ординальная индукция). Для любого ординала $\beta$, парадокс $\beta$-модального ранга требует $\mathrm{md}^\omega(\alpha_P) > \beta$, что определяется ординальной структурой $\mathrm{md}^\omega$. T-2f\*\*\* строгое неравенство гарантирует блокировку на каждом ординальном уровне. ∂

**Шаг 3** (согласованность). $\mathrm{md}^\omega$ ограничен снизу классом ординалов $\leq \kappa_2$ (по (R3) ZFC + 2-inacc). Все парадоксы, требующие $\mathrm{md}^\omega > \kappa_2$, лежат вне R-S-стратума по 134.T. ∎

**Лемма 136.L1** *(T-2f\*\*\* ⇒ T-2f\*\* ⇒ T-2f\*)*. На предикатах с конечным модальным рангом $\mathrm{md}^\omega = \mathrm{md}$, и T-2f\*\*\* совпадает с T-2f\*\*. По 130.L1, T-2f\*\* ⇒ T-2f\*. Транзитивность. ∎

**Лемма 136.L2** *(сохранение трансфинитной провабильности GL_ω)*. T-2f\*\*\* блокирует только md-нерастущие трансфинитные фиксточки. Расширенные провабильность-логики типа GL_ω (Solovay-style ω-rule) используют диагональную лемму на md^ω-стратифицированном языке и **не** блокируются.
