...
- **Axi-1** (структура $\langle\!\langle \cdot \rangle\!\rangle$): локальная малость в каждом $\mathbf{U}_k$-слое (стандартно). Внутренняя замкнутость требует $(\infty, 2)$-sheafification:
  ...
  - Для (∞,2)-обобщения: используется конструкция Verity 2008 «Weak complicial sets I: Basic homotopy theory» + Riehl-Verity 2022 «Elements of ∞-Category Theory» §4 для $(\infty, 2)$-стеков. Условие descent на 2-уровне выполнено через hyperdescent (Riehl-Verity 2022 §4 «Cosmological functors and homotopy 2-categories» — стандартный результат для cocomplete ∞-космоев).
...
**Лемма 136.L_R** *(согласованность md^ω со стек-моделью 131.T).* $\mathrm{md}^\omega$ ограничен сверху ординалом $\kappa_2$ для R-S-стратифицированных артикуляций.

*Доказательство.* По 131.T: R-S-метатеории живут в $\mathbf{U}_2$-моделируемом универсуме с $\kappa_2$-presentability через стек-модель.

**Уточнение распределения $\mathrm{md}^\omega$ по κ-уровням** (closing R3↔R10 gap):
- Для $\alpha \in \mathfrak{M}^\mathrm{stack}_{\mathrm{Diak}, \kappa_1}(\mathbf{U}_1)$: $\mathrm{md}^\omega(\alpha) \leq \kappa_1$. Предикаты $L^\omega_\alpha$ для таких $\alpha$ имеют md^ω в $[0, \kappa_1)$.
- Для $\alpha \in \mathfrak{M}^\mathrm{stack}_{\mathrm{Diak}, \kappa_2}(\mathbf{U}_2) \setminus \mathbf{U}_1$: $\mathrm{md}^\omega(\alpha) \in [\kappa_1, \kappa_2)$. Это — $\mathsf{M}^\mathrm{stack}$-образы $\mathbf{U}_1$-артикуляций.

**Переход через $\mathsf{M}$**: метаизация $\mathsf{M}^\mathrm{stack}: \mathbf{U}_1 \to \mathbf{U}_2$ (Лемма 131.L1) увеличивает md^ω на ровно $\kappa_1$ (один шаг универсального восхождения). Это согласовано с $\mathrm{md}^\omega(\Box P) = \mathrm{md}^\omega(P) + 1$ для модальностей: $\mathsf{M}$ — это именно модальность «metaize», и её ранг канонически 1 на $\mathbf{U}_1$-уровне, $\kappa_1$ при переходе $\mathbf{U}_1 \to \mathbf{U}_2$.

**Стабилизация по Drake reflection (Лемма 131.L3)**: внутри $\mathbf{U}_2$ дальнейшие модальности добавляют md^ω без выхода за $\kappa_2$ — Drake reflection обеспечивает self-representation на одном κ-уровне.

Заключение: $\mathrm{md}^\omega(\alpha) \leq \kappa_2$ для всех $\alpha \in \mathfrak{M}^\mathrm{stack}_\mathrm{Diak}$. ∎
