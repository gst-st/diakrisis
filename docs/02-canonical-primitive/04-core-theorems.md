...
**Теорема 10.T1**: Система Axi-0..Axi-9 + T-α + T-2f\* (12 аксиом без Axi-8) имеет модель в Cat (2-категория малых категорий).

**Статус**: [Т]. Модель предъявляется конструктивно:
- ⟪⟫ := Cat.
- 𝖬 := конкретный доступный эндофунктор.
- α_math := конкретная малая категория.
- ⊏_κ — через морфизмы Cat.

**Последствие**: Diakrisis-без-Axi-8 консистентна относительно ZFC + 2 недостижимых.

**Различение consistency claims**:
- **Diakrisis-без-Axi-8 (12 аксиом)**: Cat-модель ⟹ relative consistency vs ZFC + 2-inacc (10.T1).
- **Полная Diakrisis (13 аксиом, включая Axi-8 и T-2f\*\*)**: $(\infty,2)$-стек-модель $\mathfrak{M}^\mathrm{stack}_\mathrm{Diak}$ ⟹ relative consistency vs тот же ZFC + 2-inacc (теорема 131.T).

**Ограничение Cat-модели**: Axi-8 (M-5w\*) **нарушается** — α_𝖬 Ёнеда-представим в Cat (это LP-категория, по 14.T2 α_𝖬 представим). Cat-модель свидетельствует о консистентности 12-аксиомной части, не о нетривиальности всей системы. Полная Diakrisis (с Axi-8 нетривиально удовлетворённой) реализуется в стек-модели через восхождение по универсумам на объектном уровне.
...
**14.T1**: существуют активные артикуляции (непредставимые через Ёнеду).

**14.T2**: если ⟪⟫ **не** локально-представима (не-LP), 𝖬 непредставим; α_𝖬 активна.
...
### 8.2 Связь 14.T2 с Axi-8

14.T2 — *условная теорема* (conditional), которая преобразует экзистенциальную аксиому Axi-8 в формальное следствие при выполнении условия «⟪⟫ не-LP»:
- **Cat-модель** (LP): 14.T2 не применима; Axi-8 нарушается тривиально (α_𝖬 представим).
- **Стек-модель** $\mathfrak{M}^\mathrm{stack}_\mathrm{Diak}$ (не-LP, по 131.C2): 14.T2 даёт реализацию Axi-8 как теоремы; α_𝖬 активна через κ-стратификацию.

Активные артикуляции — **подтверждённая** (а не «потенциальная») область нетривиальности Diakrisis в стек-модели (131.T). По AFN-T полное удовлетворение всех 13 аксиом *в абсолютной форме без κ-стратификации* невозможно; стек-модель решает это через restricted κ-башню.
...
| 43.T1 | [Т-набр] | Классифицирующее пространство |
