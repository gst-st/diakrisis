...
- **2-категория**: минимальный уровень, дающий gauge-структуру через автоэквивалентности; минимальный рабочий уровень для формализации в стандартных prover'ах (Lean, Coq, Agda).
- **(∞,1)-категория**: богатая версия (Lurie HTT), даёт (∞,1)-Diakrisis — эквивалентна 2-Diakrisis через τ_{≤2}-truncation.
- **(∞,∞)-категория**: максимальная higher-coherent версия, даёт (∞,∞)-Diakrisis; абсолютно необходима для полной формулировки 59.T.

Выбор **2-категории** для изложения — pragmatic choice. Все центральные результаты (10.T–19.T, 55.T, TH-Final) переносятся на каждый уровень n через n-truncation (60.T). Параметризация по n: [/06-limits/06-absoluteness](/06-limits/06-absoluteness).
...
- ⟪⟫ с внутренней замкнутостью — **близка** к 2-топосной теории Шульмана (Shulman, 2-topoi).
- Полной эквивалентности со 2-топосом Шульмана **нет** (в 2-топосе требуется больше структуры — subobject classifier, exponentials); у нас — минимальная форма замкнутости.
...
**Замечание**: в framework-корпусе Trace(𝖠) часто использовался как первичный объект. В Diakrisis он — **производный** по П-0.2 (экономия).
