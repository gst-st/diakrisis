...
- **(∞,1)-категория**: богатая версия (Lurie HTT), даёт (∞,1)-Diakrisis — эквивалентна 2-Diakrisis через τ_{≤2}-усечение.
- **(∞,∞)-категория**: максимальная higher-когерентный версия, даёт (∞,∞)-Diakrisis; абсолютно необходима для полной формулировки 59.T.

Выбор **2-категории** для изложения — прагматический выбор. Все центральные результаты (10.T–19.T, 55.T, TH-Final) переносятся на каждый уровень n через n-усечение (60.T). Параметризация по n: [/06-limits/06-absoluteness](/06-limits/06-absoluteness).
