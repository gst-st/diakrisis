...
**Применение**: альтернативные основания (HoTT, NCG) — тоже калибровочно-эквивалентные ZFC + variations, не глубже.
...
| Morita-эквивалентность слишком грубая → Уровень 6 интенсионально | Потеря интенсиональный-структуры = выход за уровень | 98.T + 99.T: функтор $\mathbf{I}: \langle\!\langle \cdot \rangle\!\rangle^\mathrm{op} \to \mathcal{S}_\mathrm{int}$ через дисплейные 2-семейства; срез-локальность — образ в слоях $\widetilde{\pi}^{-1}([\alpha])$ над $\mathcal{M}_\mathrm{Fnd}$, не в базе |
...
3. В `/06-limits/08-intensional-refinement` доказаны 98.T (существование функтора **I**) и 99.T (срез-локальность) — полное формальное закрытие единственного исторически-открытого зазора.
4. В `/10-reference/02-theorems-catalog` присутствует навигационная карта защит с обновлёнными статусами.

**Превентивный принцип**: любой новый обходной путь вокруг AFN-T должен сначала пройти проверку по таблице «путь → теорема-защита» в [`/06-limits/02-th-final#три-обходных-пути`](/06-limits/02-th-final#три-обходных-пути) и по препринту [MSFS](/10-reference/04-afn-t-correspondence) §8 (Theorems `thm:universe`, `thm:reflective`, `thm:I-existence`, `thm:срез-локальность`). Если защита найдена — путь закрыт ссылкой. Если не найдена — путь легитимен и требует содержательного математического ответа (Question 6 препринта).
