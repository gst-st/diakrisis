...
# 15 негативных уроков из seed-работы и корпусной рефлексии
...
NL-1..NL-14 извлечены из анализа 13 исторических попыток построить уровень 6. NL-15 фиксирует мета-паттерн атак на TH-Final без полной проверки каталога защит — методологический урок о навигационной целостности корпуса.
...
**NL-15**: навигационная целостность корпуса. **Применимость**: доказанные защиты TH-Final должны быть не только существующими в корпусе, но и видимыми на первом уровне чтения через явную карту «обходной путь → теорема-защита».
...
**Канонический набор обходных путей вокруг TH-Final и их корпусные контраргументы**:
...
| Morita-эквивалентность слишком грубая → Уровень 6 интенсионально | Потеря intensional-структуры = выход за уровень | 98.T + 99.T: функтор $\mathbf{I}: \langle\!\langle \cdot \rangle\!\rangle^\mathrm{op} \to \mathcal{S}_\mathrm{int}$ через дисплейные 2-семейства; slice-locality — образ в слоях $\widetilde{\pi}^{-1}([\alpha])$ над $\mathcal{M}_\mathrm{Fnd}$, не в базе |

**Три обходных пути и их структурный статус**:
1. Экстенсиональный коллапс Morita-редукции (интенсиональный refinement) — ✅ formally closed (98.T + 99.T).
2. Универс-полиморфизм без супремума (proper-class structures) — ✅ formally closed.
3. Трансметатеоретическая рефлексивная башня ($S + \mathrm{Con}(S) + \ldots$) — ✅ formally closed.

**Корпусная реализация NL-15**:

1. В `/06-limits/02-th-final` введён раздел «Стандартные обходные пути и их закрытие» с явной картой: обходной путь → конкретные защитные теоремы. Все три пути — ✅ formally closed.
2. В `/10-reference/03-gap-status` обходные пути вокруг TH-Final отмечены как **formally closed** с указанием защитных теорем.
...
**Превентивный принцип**: любой новый обходной путь вокруг TH-Final должен сначала пройти проверку по таблице «путь → теорема-защита» в `/06-limits/02-th-final#стандартные-обходные-пути-и-их-закрытие`. Если защита найдена — путь закрыт ссылкой. Если не найдена — путь легитимен и требует содержательного математического ответа.
