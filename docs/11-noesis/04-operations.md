...
**52 конечных точки** в 8 группах (была 46 в 7 группах — расширено
группой `ontology/*` после поставки Verum OWL 2 V1; см.
`24-owl2-integration.md`).
...
| **Агент** | 4 | Ведомые LLM |
| **Онтология (OWL 2)** | 6 | OWL 2 import/export/classify (см. 24-owl2-integration.md) |
| **Итого** | **52** | Полный API |

Плюс пакетные операции + подписки + вебхуки.

### Группа «Онтология» (новая, шипится с Verum VUVA C7+C8+C9 V1)

| Endpoint | Метод | Описание |
|---|---|---|
| `ontology/import` | POST | импорт OWL 2 Functional Syntax → knowledge object |
| `ontology/export` | POST | экспорт knowledge object → OWL 2 Functional Syntax (`.ofn`) |
| `ontology/classify` | POST | классификационная иерархия (subclass closure + violations) |
| `ontology/subsume` | POST | проверка `C ⊑ D` (формальная подкласс-обязанность) |
| `ontology/instance` | POST | проверка `a : C` |
| `ontology/consistency` | POST | DL-выполнимость (циклы + disjoint/subclass конфликты) |

Routing: эти endpoints делегируют функциям с `@enact(epsilon:
"ε_classify")` в потребляющем knowledge object. Семантика
(CWA по умолчанию + OWA opt-in, IRI scheme,
operator coverage) — см. `24-owl2-integration.md`.
