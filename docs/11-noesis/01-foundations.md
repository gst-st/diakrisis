...
**Теорема NO-2 (консервативность над Diakrisis)** [Т·L1]: Все 129 теорем Diakrisis 10.T1..97.T **переносятся** на Noesis.Core через канонический функтор включения Noesis ↪ Diakrisis.
...
**TH-Final ABSOLUTA_TOTALIS** — пятиосевая абсолютность no-go-результата:
...
**Ключевое свойство**: большинство теорем Diakrisis применяются **автоматически** через аннотации Verum `@verify(proof)`. Пользователь не обязан вручную проверять TH-Final ABSOLUTA_TOTALIS — SMT-бэкенд применит её.
