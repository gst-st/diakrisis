...
| **Инициальность + ядро + не-Yanofsky + Axi-8** | Открытые вопросы программного слоя после 106.T: (1) ядро $\mathrm{Aut}_2(\langle\!\langle \cdot \rangle\!\rangle) \to \pi_0\mathrm{Aut}_2(\mathfrak{M}_\mathrm{Fnd})$; (2) явное построение второго представителя $\mathcal{L}_\mathrm{Cls}^\top$; (3) парадоксы вне Yanofsky (Berry, Лёб, паранепротиворечивые); (4) Axi-8 в не-LP моделях. | ✅ **формально закрыто** | **128.T** + **129.T** в [`/06-limits/10-maximality-theorems`](/06-limits/10-maximality-theorems); **130.T** (T-2f\*\*) + **131.T** ((∞,2)-стек модель) в [`/02-canonical-primitive/02-axiomatics`](/02-canonical-primitive/02-axiomatics) |
