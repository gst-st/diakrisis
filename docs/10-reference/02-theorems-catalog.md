...
Формальные доказательства теорем граничной леммы AFN-T (α, β, combined), пяти осей абсолютности, обход-paths и meta-классификации уровень 5+ **вынесены в препринт [MSFS](/10-reference/04-afn-t-correspondence)** (`internal/math-msfs/paper-en/paper.tex`). Diakrisis-каталог сохраняет внутреннюю нумерацию и контекст; полные формальные изложения — в препринте. Таблица соответствия `N.T` ↔ `thm:...`: [`/10-reference/04-afn-t-correspondence`](/10-reference/04-afn-t-correspondence).
...
- **16.T1** [Т·L1] — Z_1 ≃ Z_2 ≃ Z_3 через 2-коммутативный треугольник 2-функторов Φ_{12}, Φ_{23}, Φ_{31} с условием 2-коцикла Φ_{31} ∘ Φ_{23} ∘ Φ_{12} ≃_2 id. Явная конструкция всех трёх 2-функторов через (1) Ловер-диагонализацию + 13.T/17.T, (2) Ёнеда-2-преобразования + 14.T1, (3) Ёнеда-расширение + доступность. Существенная сюръективность и полнота-верность каждого Φ_{ij} — через Бек–Шевалли свойство пределов. Полное доказательство: [/02-canonical-primitive/04-core-theorems#эквивалентность-z](/02-canonical-primitive/04-core-theorems#эквивалентность-z).
...
- Φ корректно определена (калибровочно-инвариантна через 29.T).
- Φ существенно сюръективна (29.T даёт α для каждого F).
- Φ полностью точна на 2-уровне (30.T — обратное).
...
- **AFN-T** [Т·L1] — унифицированная no-go (AFN-T (α-часть) ∧ AFN-T (α-часть)-extended).
  → MSFS Theorem `thm:afnt-alpha`. Следствие: $\mathcal{L}_6 = \emptyset$ (Proposition `prop:level-structure`(iv)).
...
**AFN-T**: унификация — граничная лемма. Пять осей абсолютности — пятиосевая абсолютность AFN-T (MSFS Theorem thm:horizontal–thm:completeness), с логической зависимостью осей через Remark (removed in current MSFS state).
...
- **Def R-S** [О] — Reasonable Rich-Metatheory: условия (R1) арифметика, (R2) r.e. аксиоматизация, (R3) непустая модель, (R4) Гёдель-кодирование, (R5) категорная интерпретация.
...
- **59.T** [Т·L2] — (∞,∞)-AFN-T: абсолютность в (∞,∞)-контексте (прямое доказательство через 67.T). → MSFS Theorem `thm:direct-infty` (для классических $S$) + Remark `rem:direct-infty-scope` (для конструктивных $S$ — main Theorem `thm:afnt-alpha`).
...
- **61.T** [Т·L2] — α_inf-cat: новая артикуляция, ν = Ω (через 94.T). → MSFS `thm:universe` (частный случай).
...
После 84.T и 87.T трёхосевая форма (S, n, μ) расширена до пятиосевой **пятиосевая абсолютность AFN-T** (S, n, μ, ξ, π); см. группу 84.T и MSFS Theorem thm:horizontal–thm:completeness. Трёхосевая форма сохраняется как частный случай (без латеральной и completeness осей).
...
**97.T** [Т·L1] — в субструктурной метатеории S' следующие эквивалентны: (a) S' содержит `!`, (b) S' выражает PA, (c) Π_3-max достижимо, (d) S' ∈ R-S.

**Следствие**: без `!` — ν(α_S') ≤ ω, Π_3-max принципиально недостижимо; субструктурные-без-`!` системы структурно вне Diakrisis.
...
Формальное закрытие вопроса о самоклассификации Diakrisis в пространстве уровень 5+ meta-structures. Детали: [/06-limits/09-meta-classification](/06-limits/09-meta-classification).
...
- **109.T** [Т·L3] — **Дуал-AFN-T (абсолютно-акт-no-go)**: не существует $\varepsilon_\infty$ одновременно scope/depth/meta/gauge/performance-абсолютного. Пятиосевая абсолютность переносится на ДЦ-сторону через $\varepsilon$-функтор. Формально в MSFS Theorem~
ef{thm:dual-afnt} + Theorem~
ef{thm:dual-five-axis}. Ловер-scope $\mathrm{LS}(\cE)$ обобщён до **closed symmetric monoidal** (включая $*$-autonomous) через универсальную диагональ Яновский — покрывает linear logic, ludics, квантовые enactments, resource-sensitive type theories. См. [`/12-actic/05-dual-afn-t`](/12-actic/05-dual-afn-t).
...
- **125.T** [Т·L3] — **Метастемология Е. Чурилова имеет $\varepsilon = \omega \cdot 2 + 1$** — уровень ДЦ-гибридной практики со стабилизированной методологией без полного институционального самовоспроизводства; Актика ($\varepsilon = \omega \cdot 3$) располагается на два слоя $\varepsilon$-стратификации над ней — различие уровней формальной замкнутости (встроенная no-go-рефлексия 109.T), а не оценочное суждение.
...
### L3 — Закрытие открытых вопросов после 106.T (128.T–131.T)

- **128.T** [Т·L3] — **Структура ядра калибровочной сюръекции**: $K = \ker[\mathrm{Aut}_2(\langle\!\langle \cdot \rangle\!\rangle) \twoheadrightarrow \pi_0\mathrm{Aut}_2(\mathfrak{M}_\mathrm{Fnd})] \simeq_2 \mathrm{Aut}_2^{\mathrm{калибр\text{-}тожд}}(\langle\!\langle \cdot \rangle\!\rangle_{\mathrm{nR\text{-}S}})$ + расщепляемая точная последовательность $1 \to K \to \mathrm{Aut}_2(\langle\!\langle \cdot \rangle\!\rangle) \to \pi_0\mathrm{Aut}_2(\mathfrak{M}_\mathrm{Fnd}) \to 1$ через 104.T-сечение. → [`/06-limits/10-maximality-theorems`](/06-limits/10-maximality-theorems) §128.T.
- **129.T** [Т·L3] — **Инициальность Diakrisis в $\mathcal{L}_\mathrm{Cls}^\top$**: для каждого $\mathbf{F} \in \mathcal{L}_\mathrm{Cls}^\top$ над ZFC + 2-inacc существует канонический 2-функтор $\Phi_\mathbf{F}: \mathrm{Diakrisis} \to \mathbf{F}$, единственный с точностью до $(\infty, 2)$-изоморфизма; $\Phi_\mathbf{F}$ — $(\infty, \infty)$-эквивалентность. Растворяет вопрос «второго представителя» через каноническую жёсткость. Лемма 129.L1 (совместная точность $(\mathrm{Cl}, \mathbb{I})$). → [`/06-limits/10-maximality-theorems`](/06-limits/10-maximality-theorems) §129.T.
- **130.T** [Т·L3] — **T-2f\*\* модальная стратификация**: усиление T-2f\* условием $\mathrm{md}(P) < \mathrm{md}(\alpha_P)$ (модально-определимостный ранг) блокирует Berry, парадоксальное применение Лёба, паранепротиворечивый Curry — расширение универсальной парадокс-иммунности 105.T за пределы Yanofsky-сводимости. T-2f\*\* ⇒ T-2f\* (130.L1); стандартная теорема Лёба GL/PA не блокируется (130.L2). → [`/02-canonical-primitive/02-axiomatics`](/02-canonical-primitive/02-axiomatics) §T-2f\*\*.
- **131.T** [Т·L3] — **Реализация Axi-8 в (∞,2)-стек модели**: в модели $\mathfrak{M}^\mathrm{stack}_\mathrm{Diak}$ ($(\infty, 2)$-стек Rich-оснований над $\mathrm{Syn}(S)$) Axi-8 (M-5w\*) выполнена нетривиально через восхождение по универсумам на объектном уровне ($\kappa_1 \to \kappa_2$, ограниченное двумя итерациями + стабилизация на теоретическом уровне 102.T-(a)). Cat-модель 10.T1 — $\kappa_1$-усечение стек-модели. → [`/02-canonical-primitive/02-axiomatics`](/02-canonical-primitive/02-axiomatics) §Axi-8.
...
Все три стандартных обходных пути формально закрыты; самоклассификация Diakrisis в пространстве уровень 5+ мета-структур также формально закрыта. Корпус Diakrisis полностью устойчив к стандартным атакам на AFN-T: на экстенсиональном уровне — 5-осевая абсолютность (S, n, μ, ξ, π); на интенсиональном уровне — срез-локальность 99.T; на уровне мета-классификации — стабилизация 102.T. Детальное обсуждение: [/06-limits/02-th-final#три-обходных-пути](/06-limits/02-th-final#три-обходных-пути). Полные формальные доказательства — препринт [*MSFS*](/10-reference/04-afn-t-correspondence) §8.
