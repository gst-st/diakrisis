...
- **137.T** [Т·L3] — **Weak-AFN-T для $\mathcal{L}_\mathrm{Fnd}^\mathrm{weak}$**: дуальная no-go-теорема для bounded-arithmetic стратума. $\mathcal{L}_\mathrm{Abs}^\mathrm{weak} = \emptyset$ через ослабленные R-S\* условия + Simpson 2009 reverse-mathematics. Параллельна классической AFN-T, не выводима из неё. Сила: Con(weak-AFN-T) = Con(ZFC + 1-inacc). → [`/06-limits/05-what-remains-possible`](/06-limits/05-what-remains-possible) §137.T.
