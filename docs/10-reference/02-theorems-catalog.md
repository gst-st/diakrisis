...
Формальные доказательства теорем граничной леммы AFN-T (α, β, combined), пяти осей абсолютности, обходных путей и мета-классификации уровень 5+ **вынесены в препринт [MSFS](/10-reference/04-afn-t-correspondence)** ([Zenodo](https://zenodo.org/records/19755781)). Diakrisis-каталог сохраняет внутреннюю нумерацию и контекст; полные формальные изложения — в препринте. Таблица соответствия `N.T` ↔ `thm:...`: [`/10-reference/04-afn-t-correspondence`](/10-reference/04-afn-t-correspondence).
...
- **10.T5** [Т·L1] — существование Fix(𝖬) при доступности 𝖬.
...
- **Значение**: калибровочной свобода структурно встроена.
- **История**: раньше — Ax-5.

**10.T5 — Существование Fix(𝖬)**:
- **Формулировка**: Fix(𝖬) ≠ ∅ при доступности 𝖬.
...
- **11.T3** [Т·L2] — Асимптотический побег (конечный случай, через диагонализацию типа леммы Кёнига).
...
- **12.C1** — связь Z с побегом.

### Детализация 12

**12.T1**: A_init = colim 𝖬^κ(⊥) при доступности.

**12.T2**: A_fin = lim 𝖬^κ(⊤).

**12.C1**: связывает Z-характеризации с траекторией побега.

## Побег (13, 17)

- **13.T** [Т·L1] — строгий теорема о побеге (переупаковка Гёдель II в 𝖬-контексте).
- **17.T** [Т·L1] — трансфинитный побег (по индукции на ординалах + доступность).
...
## Алгоритмическая gauge-эквивалентность (16-gauge-decision)

- **Теорема 16.5** [Т·L3] — Разрешимость gauge-эквивалентности для конечно-аксиоматизированных R-S за $O(2^{O(|\alpha|)})$. → [`/03-formal-architecture/16-gauge-decision`](/03-formal-architecture/16-gauge-decision) §3.
- **Теорема 16.6** [Т·L3] — Полу-разрешимость ($\Sigma_1$) для рекурсивно-аксиоматизированных R-S. → §4 там же.
- **Теорема 16.7** [Т·L3] — Нижняя граница: word problem reduction (Novikov-Boone), универсальная неразрешимость. → §4 там же.
- **Теорема 16.10** [Т·L3] — Round-trip property для 108.T: `canonicalize(inverse(translate(α))) = canonicalize(α)`. → §5 там же.

### Детализация

**16.5–16.7**: алгоритмическая теория gauge-эквивалентности с точными границами разрешимости. Конечно-аксиоматизированные R-S — разрешимы за single-exponential; общий случай — Тьюринг-эквивалентен word problem для finitely-presented групп.

**16.10**: round-trip 108.T — это операциональная реализация теоремы 108.T в Verum (Шаг 10 интеграционного плана). Закрывает критический gap для двойной OC/DC stdlib.
...
- **43.T1** [Т·L2] — Классифицирующее пространство: Trace(𝖠)/gauge ≃ 𝓜_Fnd как 2-stack. → Препринт Convention `conv:notation` ($\fM$ как straightening of Cartesian fibration, Люри HTT §3.2).
...
1. F — Rich formal system → по имеет категорную семантику: существует локально-малая 2-категория Mod(F) моделей F с F-preserving 2-functorами.
2. Определим **представляющий 2-functor**: `Ψ_F : ⟪⟫^op → Cat`, `Ψ_F(β) := Hom_2(β, y(F))`, где y(F) — канонический объект в 2-presheaves.
3. Ψ_F — accessible (по + R5).
...
- **(BF1) Содержит тождества**: $\mathrm{id}_\alpha \in \mathcal{W}$ для всякого α. ✓ (тождество — калибровочно-эквивалентность).
- **(BF2) Замкнутость под композицией**: $\tau, \sigma \in \mathcal{W} \Rightarrow \sigma \circ \tau \in \mathcal{W}$. ✓ (композиция калибровочно-эквивалентностей — gauge).
- **(BF3) 2-pullback-стабильность**: для $\tau \in \mathcal{W}$ и произвольного $f$, 2-pullback $\tau \times_{\_} f$ содержит калибровочно-эквивалентность над f. ✓ (2-pullback от gauge-изоморфизма — gauge-изоморфизм; 2-categorical результат Келли 1982 §2.1).
- **(BF4) 2-правая стрелка**: для $f: A \to B$ и $\tau \in \mathcal{W}_B$ существует $g: A' \to A$ и $\sigma \in \mathcal{W}_{A'}$ с $f \circ g \simeq \tau \circ f'$ для некоторого $f'$. ✓ (следует из доступности Axi-4 + 29.T functoriality).
...
**AFN-T (α-часть)-extended**: (β)-часть — limit-approach не работает (провал Пути Ε). Механизм: argumenta-доступности (Адамек–Росицкий Thm 2.11, 2.45); proper-class-башни исключены через (R2) r.e.-аксиоматизации (MSFS Proposition `prop:proper-class` + Corollary `cor:no-proper-class`).
...
- **33.T3** [Т·L2] — Giry-монада в Diakrisis как конкретный экземпляр probability monad (Giry 1982 + доступность).
...
- **48.T** [Т·L2] — α_linear без `!` недостаточна для Rich (через 97.T-компромисс).
...
- **Def α_Д-linear** [О] — линейная логика articulation: ν X. (X ⊗ X ⊸ !X).
...
## 97.T — Компромисс линейности и генеративности
...
- **99.T** [Т·L2] — срез-локальность. 2-Гротендик-фибрация $\widetilde{\pi}: \mathcal{S}_\mathrm{int} \to \mathcal{M}_\mathrm{Fnd}$ с $\pi = \widetilde{\pi} \circ \mathbf{I}$. Образ $\mathbf{I}$ проецируется на уже существующие точки $\mathcal{M}_\mathrm{Fnd}$. → MSFS Theorem `thm:срез-локальность` + Corollary `cor:slice-level`.
...
## Мета-классификация уровень 5+ (100.T–102.T)
...
- **Def $\mathfrak{Meta}_{5+}$** [О] — класс мета-структур уровня 5+: (M1) локальная малость, (M2) классификационный функтор, (M3) доступная рефлексия, (M4) непорождающая, (M5) параметризованная метатеорией. → MSFS Definition `def:meta`.
- **Def $\mathfrak{Meta}_{5+}^{\max}$** [О] — подкласс максимальных: (Max-1) полная классификация, (Max-2) калибровочная полнота, (Max-3) стратификация по глубине (формализована MSFS как единая аксиома доступной 2-теории, блокирующая Рассела/Curry/Grelling/Burali-Forti/Жирара — см. `rem:max3-парадокс-иммунность`), (Max-4) интенсиональная полнота. → MSFS Definition `def:maximality`.
...
- **102.T** [Т·L2] — Мета-классификация стабилизация. $\mathfrak{M}^{(5+ \cdot 2)} \simeq_2 \mathfrak{M}^{(5+)}$, no уровня 6 escalation. Формальное доказательство через вложение $\mathfrak{Meta}_{5+} \hookrightarrow \Pi_{(\infty, \infty)}$ + $(\infty, \infty)$-стабилизация (Барвик–Schommer-Pries). → MSFS Theorem `thm:meta-stab`.

## Доказательства максимальности (103.T–106.T, Diakrisis-only)
...
- **103.T** [Т·L3] — **Универсальная артикуляция (Max-1)**. Каждая $S \in \mathrm{R\text{-}S}$ допускает каноническую артикуляцию $\alpha_S = (\mathrm{Syn}(S), \mathsf{M}_S) \in \langle\!\langle \cdot \rangle\!\rangle$; функтор $\mathrm{Artic}: \mathcal{F} \to \langle\!\langle \cdot \rangle\!\rangle$ существенно сюръективен на $\mathfrak{M}_\mathrm{Fnd}$. Доказательство через Линденбаума–Тарского (Сили 1984, Хофман 1997, Капулкин–Ламсдейн 2021) + доступность метаизации (Адамек–Росицкий 1994, Лемма 103.L1). ⇒ $\mathrm{image}(\mathrm{Cl}_\mathrm{Diakrisis}) = \mathfrak{M}_\mathrm{Fnd}$.
- **104.T** [Т·L3] — **Калибровочная полнота (Max-2)**. $\mathrm{Aut}_2(\langle\!\langle \cdot \rangle\!\rangle) \twoheadrightarrow \pi_0 \mathrm{Aut}_2(\mathfrak{M}_\mathrm{Fnd})$: каждая Морита-эквивалентность R-S реализуется 2-автоэквивалентностью метакатегории. Поднятие $\sigma \mapsto \tilde\sigma$ через 103.T + коммутация с $\mathsf{M}$ (Лемма 104.L1).
- **105.T** [Т·L3] — **Универсальная парадокс-иммунность (Max-3)**. T-2f\* блокирует *все* Яновский-сводимые самореферентные парадоксы (Яновский 2003). Доказательство: глубина экспоненциала $\mathrm{dp}(T^Y) = \mathrm{dp}(Y)+1$ + T-2f\* строгое неравенство ⇒ слабая точечная сюръективность $\alpha: Y \to T^Y$ невозможна ⇒ все диагональные конструкции заблокированы. Расширяет 18.T с 5 семейств до *универсальной* иммунности.
- **106.T** [Т·L3] — **Сводная: Diakrisis $\in \mathcal{L}_{\mathrm{Cls}}^{\top}$**. Все 4 условия max-1..4 выполнены: (Max-1) по 103.T, (Max-2) по 104.T, (Max-3) по 105.T, (Max-4) по 99.T. ⇒ Diakrisis принадлежит максимальному подклассу мета-классификаторов как **теорема**, не программа.
  ...
  - **106.C4** 102.T применим: итерированная мета-классификация Diakrisis стабилизируется на на теоретическом уровне, с восхождение по универсумам.
...
ef{thm:dual-five-axis}. условия Ловера $\mathrm{LS}(\cE)$ обобщён до **closed symmetric monoidal** (включая $*$-autonomous) через универсальную диагональ Яновский — покрывает линейная логика, ludics, квантовые enactments, resource-sensitive type theories. См. [`/12-actic/05-dual-afn-t`](/12-actic/05-dual-afn-t).
...
- **124.T** [Т·L3] — **Сопряжение $\mathsf{M} \dashv \mathsf{A}$ через 108.T**: 2-categorical biadjunction в смешанной 2-категории, индуцированной 108.T-эквивалентностью $\varepsilon \dashv \alpha$. Формально: (OC-сторона) $\mathsf{M} \dashv \widetilde{\mathsf{A}}$ в $\langle\!\langle \cdot \rangle\!\rangle$ + (DC-сторона) $\widetilde{\mathsf{M}} \dashv \mathsf{A}$ в $\rangle\!\rangle \cdot \langle\!\langle$ — оба эквивалентны через 108.T. Доказательство — Шаги A–D + (DC-сторона) в [`/12-actic/06-actic-theorems`](/12-actic/06-actic-theorems) §14.2.2–§14.2.3.
...
- **130.T** [Т·L3] — **T-2f\*\* модальная стратификация**: усиление T-2f\* условием $\mathrm{md}(P) < \mathrm{md}(\alpha_P)$ (модально-определимостный ранг) блокирует Berry, парадоксальное применение Лёба, паранепротиворечивый Curry — расширение универсальной парадокс-иммунности 105.T за пределы Яновский-сводимости. T-2f\*\* ⇒ T-2f\* (130.L1); стандартная теорема Лёба GL/PA не блокируется (130.L2). → [`/02-canonical-primitive/02-axiomatics`](/02-canonical-primitive/02-axiomatics) §T-2f\*\*.
...
### L3 — Закрытие MSFS-уровневых открытых вопросов (133.T–135.T)

- **133.T** [Т·L3] — **Исчерпывающесть R-S-внутренних bypass-путей к AFN-T**: 6 классов конструкций (экстенсиональный, универс-полиморфный, рефлексивная башня, интенсиональный, модальный, субструктурный) исчерпывают типовые R-S-внутренние подходы; все блокируются соответствующими теоремами. Закрывает MSFS Q3 в R-S-внутренней области. → [`/06-limits/05-what-remains-possible`](/06-limits/05-what-remains-possible) §133.T.
- **134.T** [Т·L3] — **Тугость границы ZFC + 2-inacc**: $\mathrm{ZFC} + 1\text{-inacc}$ недостаточно для реализации полной Diakrisis с нетривиальной Axi-8 (R-S-силы $> \kappa_1$ требуют $\kappa_2$). Тугая нижняя граница; закрывает MSFS Q4. → [`/06-limits/05-what-remains-possible`](/06-limits/05-what-remains-possible) §134.T.
- **135.T** [Т·L3] — **Стратум слабых R-S**: bounded-arithmetic теории ($\mathsf{I}\Delta_0$, $\mathsf{S}_2^i$, $\mathsf{V}_0$) образуют $\mathcal{L}_\mathrm{Fnd}^\mathrm{weak} \subsetneq \mathcal{L}_\mathrm{Fnd}$, не Морита-плотный (не имеет полной (R1)/PA индукции по Simpson 2009). Закрывает MSFS Q5. → [`/06-limits/05-what-remains-possible`](/06-limits/05-what-remains-possible) §135.T.

### L3 — Исследовательские расширения (136.T–142.T)

- **136.T** [Т·L3] — **T-2f\*\*\* омега-модальная стратификация**: усиление T-2f\*\* до трансфинитных модальных рангов через ординально-индексированный $\mathrm{md}^\omega: \mathrm{Pred}(L^\omega_\alpha) \to \mathrm{Ord}$. Блокирует omega-модальные парадоксы ($A \leftrightarrow \bigwedge_n \Box^n A$), $\omega \cdot k$-итерации, гипер-модальные конструкции. T-2f\*\*\* ⇒ T-2f\*\* (136.L1); ω-rule расширения GL сохраняются (136.L2). Полное доказательство: Definition 136.D1 (трансфинитный модальный язык $L^\omega_\alpha$) + Лемма 136.L0 (well-founded ordinal recursion) + Лемма 136.L_rank (диагональная лемма Smoryński 1985 + Boolos 1993). Тугая граница в ZFC + 2-inacc. → [`/02-canonical-primitive/02-axiomatics`](/02-canonical-primitive/02-axiomatics) §T-2f\*\*\*.
- **137.T** [Т·L3] — **Weak-AFN-T для $\mathcal{L}_\mathrm{Fnd}^\mathrm{weak}$**: дуальная no-go-теорема для bounded-arithmetic стратума. $\mathcal{L}_\mathrm{Abs}^\mathrm{weak} = \emptyset$ через ослабленные R-S\* условия + Simpson 2009 reverse-mathematics. Параллельна классической AFN-T, не выводима из неё. После R11 расширена: 6-уровневая $\nu^\mathrm{weak}$-стратификация ($\mathrm{AC}^0 \subset \mathrm{LOGSPACE} \subset \mathrm{P} \subset \mathrm{NP} \subset \mathrm{PH} \subset \mathsf{I}\Delta_0$), bounded Cantor diagonal (Buss 1986), complexity-typed verification, дискретный скачок ν на границе weak/full (Лемма 137.L2). Сила: Con(weak-AFN-T) = Con(ZFC + 1-inacc). → [`/06-limits/05-what-remains-possible`](/06-limits/05-what-remains-possible) §137.T.
- **138.T** [Т·L3] — **Дуальное ядро калибровочной сюръекции**: ДЦ-сторонний аналог 128.T. $K^{\mathcal{E}} \simeq_2 \mathrm{Aut}_2^{\mathrm{калибр\text{-}тожд}}(\rangle\!\rangle\cdot\langle\!\langle_{\mathrm{nR\text{-}S}^{\mathcal{E}}})$, расщепляемая точная последовательность. Через 108.T: $K^{\mathcal{E}} \simeq_2 K$ канонически (138.C3). → [`/12-actic/06-actic-theorems`](/12-actic/06-actic-theorems) §15.1.
- **139.T** [Т·L3] — **Инициальность Актики в $\Meta_{\mathrm{Cls}}^{\top, \mathcal{E}}$**: дуал 129.T. Каноническая жёсткость дуального максимального подкласса; Актика — инициальный объект через дуальную выпрямку. Лемма 139.L1 (совместная точность $(\mathrm{Cl}^{\mathcal{E}}, \mathbb{I}^{\mathcal{E}})$). → [`/12-actic/06-actic-theorems`](/12-actic/06-actic-theorems) §15.2.
- **140.T** [Т·L3] — **ε-инвариант на $(\infty, \infty)$-уровне**: функтор $\mathrm{e}^\infty: \rangle\!\rangle\cdot\langle\!\langle_\infty \to \mathrm{Ord}$ как полное расширение ε. Согласованность с τ-truncations, $(\infty,\infty)$-эквивалентность сохранение, ординальная монотонность под $\mathsf{A}$, колимит-сохранение. Полное доказательство: Шаги 1–5 + 5 лемм с явной конструкцией $(\infty, \infty)$-категории актов через colim/lim adjoint tower (Лемма 140.L0), accessibility $\mathsf{A}^\infty$ (Лемма 140.L1), универсальность через BSP unicity (Лемма 140.L2). Универсальность по 108.C5 (68.T-дуал). → [`/12-actic/06-actic-theorems`](/12-actic/06-actic-theorems) §15.3.
- **141.T** [Т·L3] — **Конструктивный автопоэзис**: усиление 113.T до конструктивного существования. $\mathrm{Fix}(\mathsf{A}) \cap \rangle\!\rangle\cdot\langle\!\langle^\mathrm{biological} \neq \emptyset$ через явный witness $\varepsilon_\mathrm{auto} := \mathsf{A}^{\omega^2}(\varepsilon_\mathrm{life})$ в эффективном топосе. Полное доказательство: Шаги 1–4 + 3 леммы (141.L1 — higher-type computability $\omega^2$-итерации; 141.L2 — bisim = identity через AFA-аналог в Eff Aczel-Capretta 2017; 141.L3 — замкнутость biological category под $\mathsf{A}$). Явные σ/π-морфизмы через Drake reflection. Связь с УГМ T-211. → [`/12-actic/06-actic-theorems`](/12-actic/06-actic-theorems) §15.4.
- **142.T** [Т·L3] — **Йога-Патанджали + дзен-коаны как ε-практики**: восьмиступенчатый путь Аштанга-йоги формализуется как ε-иерархия от $\omega$ (Yama) до $\omega \cdot 3 + 1$ (Samadhi); дзен-коан — акт-артикулирующая диагональ, разрешимая только через ДЦ/ОЦ-двойственность 108.T. Расширение содержательного покрытия Актика на восточные традиции. → [`/12-actic/06-actic-theorems`](/12-actic/06-actic-theorems) §15.5.
...
- **[Т·L1] (прямые доказательства)**: ~35 теорем — канонические (10.T1–T5), побег (13.T, 17.T), α_Apeiron (19.T), Axi-независимость 2-уровень (21.T), ι-embedding (22.T), 18.T T-2f\* immunity, 97.T-компромисс.
...
- **[Т·L3] (редукция + ожидаемо-рутинная адаптация)**: ~29 теорем — включая 85.T UFH, 88.T-90.T (Lair/Ёнеда расширения на (∞,∞)), 94.T-96.T ((∞,∞)-extensions), **103.T–106.T (доказательства максимальности для Diakrisis $\in \mathcal{L}_{\mathrm{Cls}}^{\top}$)**.
...
| Мета-классификация уровень 5+ | Самоклассификация | **100.T, 101.T, 102.T** | ✅ формально закрыто |
