...
| **L2** | Redукция к конкретной внешней теореме + проверенная адаптация к Diakrisis | 52.T (Aczel M-types), 91.T (Шрайбер cohesion), 53.T (Жирар translation) |
...
4. По **2-Ёнеда (Kelly 1982, Теорема 2.4.1)**: accessible 2-functor Ψ_F представим — существует α_F ∈ ⟪⟫ с `Ψ_F(-) ≅ Hom_{⟪⟫}(-, α_F)`.
...
2. По **лемме Ёнеда для 2-категорий** (Kelly 1982): представляющие объекты равны до 2-изоморфизма.
...
- **12.T4** [Т·L2] — HoTT CHL (Awodey-Воеводский univalent CHL).
...
- **13.T5** [Т·L2] — SDG ↔ dependent теория типов через CHL (Kock-Ловер + Awodey).
...
- **59.T.2** [Т·L3] — Stabilization: иерархия (∞,n) стабилизируется в (∞,∞). → MSFS Theorem `thm:bergner-lurie-stab` (Барвик–Schommer-Pries 2021 + Bergner–Rezk 2013).
...
- **63.T** [Т·L2] — Уайтхед-критерий: эквивалентность через full Postnikov tower. → MSFS Lemma `lem:whitehead` (Appendix A) + Lurie HTT §6.5.4.
