...
- **08.T2** [Т] — существование периодических орбит: для любой κ-presentable α ∈ Ob(⟪⟫), 𝖬-итерации стабилизируются с периодом p ≤ λ_0 (Adámek-Rosický stabilization theorem для accessible endofunctor).
- **08.T3** [Т] — существование gauge-бифуркаций: для непрерывного семейства {G_t} gauge-group homomorphisms, стратификация Trace(𝖠)/G_t имеет конечное число стратов; границы стратов в параметре t — точки бифуркации (Whitney stratification theory, Mather 1970).
- **08.T4** [Т] — хаос на α_Apeiron-orbit: орбита 𝖬^κ(α_Apeiron) плотна в гомотопическом замыкании (через 19.T1 самоприменение) + accessible-nonlinear sensitive dependence — topologically chaotic в L²-gauge-метрике на ⟪⟫|_{α_Apeiron}.
- **08.T5** [Т] — Ляпунов-устойчивость α_uhm: функционал V(Γ) := ||Γ - ρ\*||²_{HS} монотонно невозрастает под ℒ_Ω = ℒ_0 + ℛ (ℒ_0 — CPTP, сохраняет trace-norm; ℛ — возвращающий к ρ\*); dV/dt ≤ 0 строго на trajectories.
...
- **12.T3** [Т] — Quantum CHL: types ↔ dagger-compact objects, terms ↔ morphisms, dagger-compatible equations ↔ 2-morphisms (Abramsky-Coecke 2004 Categorical QM + Selinger's completeness для quantum λ-calculus 2004).
...
- **50.T** [Т] — TH-Final S-relative: TH-Final параметризуется R-S (формально установлено в 55.T, §doc-relative).
- **51.T** [Т] — не существует R-S, в которой TH-Final опровергается: следует прямо из 55.T (абсолютность TH-Final) + 3.3.1 (контрапозиция). Гипотеза разрешена отрицательно.
- **52.T** [Т] — α_AFA-coalg Morita-редуцируема к Aczel's M-types (Aczel 1988 Theorem 4.7).
- **53.T** [Т] — ν(α_linear) = ω+1 (через Girard-translation → classical PA-equivalent с coinductive +1).
- **54.T** [Т] — ν(α_affine) ≤ ω; α_affine ∉ R-S (через 97.T: affine без ! не выражает PA).
...
- **57.T** [Т] — ν(α_poly-HoTT) = ω·2+1 (HoTT baseline ω+1 + universe-iter +ω); Morita-редуцируема к `lim_ℓ Fun(𝒰_ℓ, 𝒰_ℓ)` (Lurie HTT §4.2).
...
- **UFH^{(∞,∞)}** [Т] — α_uhm^{(∞,∞)} ≃_{gauge} ∫_Γ α_Д-hybrid^{!,(∞,∞)}(Γ) над Nuc(ℂ⁷) (по 79.T: UFH^{(∞,∞)} ⇒ UFH через τ_{≤2}; лифт в обратную сторону через canonical L_∞ — 65.T).
...
- **45.T3** [Т] — R^i F(α) через H^i (стандартная derived functor theory в (∞,1)-категориях).
- **45.T4** [Т] — DAG (derived algebraic geometry) через categorification: следует из 36.T1-T3.

## Сводная статистика (v0.2.0)

- **Всего теорем в каталоге**: **110+** (10.T1–T5, 11.T–45.T, 48.T–97.T).
- **[Т] (полные доказательства)**: **все 110+** — корпус полностью на строгом статусе.
- **[Т-набр]**: 0 — **все разрешены** строгими доказательствами (commit bd52546 + текущий).
- **[Программа]**: 1 — 78.T UFH Verum programme.
- **[Г]**: 0 — **все разрешены** (51.T отрицательно через 55.T; UFH^{(∞,∞)} через 79.T+65.T).
