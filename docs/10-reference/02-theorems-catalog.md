...
- **136.T** [Т·L3] — **T-2f\*\*\* омега-модальная стратификация**: усиление T-2f\*\* до трансфинитных модальных рангов через ординально-индексированный $\mathrm{md}^\omega: \mathrm{Pred} \to \mathrm{Ord}$. Блокирует omega-модальные парадоксы ($A \leftrightarrow \bigwedge_n \Box^n A$), $\omega \cdot k$-итерации, гипер-модальные конструкции. T-2f\*\*\* ⇒ T-2f\*\* (136.L1); GL_ω сохраняется как теорема (136.L2). Тугая граница в ZFC + 2-inacc. → [`/02-canonical-primitive/02-axiomatics`](/02-canonical-primitive/02-axiomatics) §T-2f\*\*\*.
