...
- **124.T** [Т·L3] — **Сопряжение $\mathsf{M} \dashv \mathsf{A}$ через 108.T**: 2-categorical biadjunction в смешанной 2-категории, индуцированной 108.T-эквивалентностью $\varepsilon \dashv \alpha$. Формально: (R9.OC) $\mathsf{M} \dashv \widetilde{\mathsf{A}}$ в $\langle\!\langle \cdot \rangle\!\rangle$ + (R9.DC) $\widetilde{\mathsf{M}} \dashv \mathsf{A}$ в $\rangle\!\rangle \cdot \langle\!\langle$ — оба эквивалентны через 108.T. Доказательство — Шаги A–D + (R9.DC) в [`/12-actic/06-actic-theorems`](/12-actic/06-actic-theorems) §14.2.2–§14.2.3 после R9.
...
- **136.T** [Т·L3] — **T-2f\*\*\* омега-модальная стратификация**: усиление T-2f\*\* до трансфинитных модальных рангов через ординально-индексированный $\mathrm{md}^\omega: \mathrm{Pred}(L^\omega_\alpha) \to \mathrm{Ord}$. Блокирует omega-модальные парадоксы ($A \leftrightarrow \bigwedge_n \Box^n A$), $\omega \cdot k$-итерации, гипер-модальные конструкции. T-2f\*\*\* ⇒ T-2f\*\* (136.L1); ω-rule расширения GL сохраняются (136.L2). Полное доказательство после R3: Definition 136.D1 (трансфинитный модальный язык $L^\omega_\alpha$) + Лемма 136.L0 (well-founded ordinal recursion) + Лемма 136.L_rank (диагональная лемма Smoryński 1985 + Boolos 1993). Тугая граница в ZFC + 2-inacc. → [`/02-canonical-primitive/02-axiomatics`](/02-canonical-primitive/02-axiomatics) §T-2f\*\*\*.
