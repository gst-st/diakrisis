...
- **141.T** [Т·L3] — **Автопоэзис как теорема существования**: усиление 111.T. $\mathrm{Fix}(\mathsf{A}) \cap \rangle\!\rangle\cdot\langle\!\langle^\mathrm{biological} \neq \emptyset$ через конструктивный witness $\varepsilon_\mathrm{auto} := \mathsf{A}^{\omega^2}(\varepsilon_\mathrm{life})$. Связь с УГМ T-211 (биологические автопоэзисы как специализации). → [`/12-actic/06-actic-theorems`](/12-actic/06-actic-theorems) §15.4.
