...
Формально: $\alpha_\mathrm{uhm} \in \mathsf{M}^{\omega \cdot 3 + 1}(\alpha_\mathrm{math}) \setminus \mathsf{M}^{\omega \cdot 3}(\alpha_\mathrm{math})$. По T-ε (дуал T-α) эта глубина — калибровочно-инвариант.
...
**Само-модель φ**:
- Функтор D(ℂ⁷) → D(ℂ⁷).
- Категорное представление самореференции.
- По T-96: ρ\* = φ(Γ) для притягивающей неподвижной точки.
...
| T-58 (7D ↔ 42D Морита) | калибровочно-эквивалентность α_7D ∼_{gauge} α_42D |
