...
По теореме 120.T: $\mathrm{Perf}(\alpha_\mathrm{linear}) \simeq \mathbf{Ludics}$ как 2-категория.
...
**Теорема 17.T1** [Т·L3]. *Для каждой монады эффекта $T$ в $\mathrm{Perf}(\alpha_\mathrm{linear})$ существует канонический функтор:*
$$
...
В $\mathrm{Perf}(\alpha_\mathrm{linear})$ — SMCC по 120.L1 (см. /12-actic/06-actic-theorems §11.1). Стандартные Verum-эффекты:
...
*Доказательство.* По 03-epsilon-invariant Definition 3.1 (определение $\mathrm{e}: \rangle\!\rangle\cdot\langle\!\langle \to \mathrm{Ord}$ через минимальную $\mathsf{A}^\beta$-итерацию): ε координата канонически определена для каждого объекта $\rangle\!\rangle\cdot\langle\!\langle$ — это базовое свойство ε-инварианта, не теорема.

Образ $\Psi_T \subset \mathrm{Perf}(\alpha_\mathrm{linear})$ — подмножество объектов с конкретной структурой strong monad. Композиция ε-инварианта с $\Psi_T$:
$$
\mathsf{e}_T(\sigma) := \mathsf{e}(\Psi_T(\sigma)) \quad \text{для } \sigma \in \mathcal{C}_T.
$$

Конкретные значения определяются monad-уровнем: для базовых эффектов (IO, Reader) — ω; для async — ω+1; для concurrent — ω+2 (по композиции с $\otimes$-структурой). ∎
...
| **Probability** (discrete / Giry) | $T_\mathrm{Prob}(A) = \mathrm{Distr}(A)$ | ✓ (для discrete; в Giry — strong на measurable spaces) | ✓ (commutative) | $\omega$ | `@effect(Prob)` |
...
## 6. Связь с другими теоремами

| Теорема | Связь |
|---|---|
| Предложение 5.1 | Через $\varepsilon \circ \mathsf{M} \simeq \mathsf{A} \circ \varepsilon$: эффекты сохраняются под метаизацией/активацией. |
| Теорема 16.10 (gauge-decision) | Cut-elimination в Ludics = canonicalize эффектов. |
| Теорема 120.T (Ludics) | $\mathrm{Perf}(\alpha_\mathrm{linear}) \simeq \mathbf{Ludics}$ — fundamental SMCC-эквивалентность. |
| Теорема 18.T1 (operational coherence) | Effects дают operational interpretation; α-аспект — статический тип-чек, ε-аспект — runtime monad execution. |
...
- [`/03-formal-architecture/16-gauge-decision`](/03-formal-architecture/16-gauge-decision) — round-trip 108.T.
- [`/12-actic/04-ac-oc-duality`](/12-actic/04-ac-oc-duality) — 108.T ε/α-двойственность.
- [`/12-actic/06-actic-theorems`](/12-actic/06-actic-theorems) §11 — 120.T Ludics .
