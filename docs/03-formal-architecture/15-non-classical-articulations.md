...
*Обоснование*: линейная логика с `!` — classical-equivalent через Girard-трансляция. Выразительна как HoTT+UA уровень. ∎

### Связь с TH-Final

По 50.T (S-relative): TH-Final применима к linear+! через трансляция. По 55.T (абсолютность): α_linear — **не** уровень 6, что согласуется с R-S_linear+! ∈ R-S.
...
- **Voevodsky** (UniMath project): реализация.
...
**Семантическая модель** (Lurie HTT + cumulative расширение):
...
- Translation: 𝓜_Fnd[classical] → 𝓜_Fnd[linear+!] (Girard трансляция).
