...
\begin{abstract}
We study the $(\infty, n)$-categorical moduli space $\fM$ of formal systems (hereafter \textbf{MSFS}): the classifying $2$-stack of Morita-equivalence classes of Rich-foundations ($\LFnd$ theories satisfying (R1)--(R5)). We establish four structural results and one boundary corollary.
...
\begin{itemize}
\item Is $\fM$ stratified by genuine strata of structural novelty? We define four formal strata $\LFnd \subsetneq \LCls$ with $\LClsMax \subsetneq \LCls$ and the forbidden $\LAbs$, the last shown to be empty (\S\ref{sec:hierarchy}).
\item Is $\LCls$ plural, or unique-up-to-equivalence? We prove it is plural: the $\infty$-cosmoi of Riehl--Verity, the Univalent Foundations programme of Voevodsky, and the cohesive framework of Schreiber are pairwise non-$2$-equivalent (Theorem~\ref{thm:meta-mult}). Restriction to the maximal sub-class $\Meta_{\mathrm{Cls}}^{\top}$, however, forces categoricity (Theorem~\ref{thm:meta-cat}).
...
\item Does $\fM$ admit a maximal point? We prove it does not: the $\LAbs$ stratum --- objects simultaneously formally definable $(F_S)$, non-reducible $(\Pi_{4, S, n})$, and maximally generative $(\Pi_{3\text{-max}, S, n})$ --- is empty, by a structural boundary lemma we call the \emph{Absolute Foundation No-Go Theorem} (AFN-T; Theorem~\ref{thm:afnt-alpha}).
\end{itemize}
...
We stratify $\fM$ into four formal strata distinguished by the two structurally distinct meta-operations $\mathrm{Cls}$ (horizontal, classifying) and $\mathrm{Gen}$ (vertical, generative). The strata, defined by their categorical conditions, are:
\[
...
We define the formal \emph{strata} as classes of mathematical objects:
...
$\mathrm{Cls}(\LFnd)$ does \emph{not} embed into $\LFnd$ and does not coincide with $\LAbs$. It is therefore a distinct formal stratum, which we label $\LCls$.
\end{proposition}
...
We write $\mathrm{Stratum}(X) \subseteq \{\LFnd, \LCls, \LClsMax, \LAbs\}$ for the set of formal strata containing $X$. Multi-stratum membership is standard: e.g., Univalent Foundations belongs to $\LCls$ and its underlying $\HoTT$ to $\LFnd$.

\section{Reasonable Rich-Metatheories}\label{sec:rs}

We define the class of metatheories in which AFN-T is stated. The definition isolates the minimum structure required for a theory to be a \emph{foundation} in the sense of $\LFnd$, while remaining neutral with respect to specific axiomatic details.
...
Given an R-S $S$, we associate a class $\cS_S$ of $S$-definable structures. This class plays a central role in the statement of the $\LAbs$ non-reducibility condition.
...
We state formally the three $\LAbs$ conditions: formal definability $(F_S)$, non-reducibility $(\Pi_{4, S, n})$, and maximal generativity $(\Pi_{3\text{-max}, S, n})$.
...
Beyond the main no-go result, we establish a structure theorem for $\LCls$ --- the class of meta-frameworks classifying $\LFnd$ foundations. The results of this section establish that $\LCls$ is structurally plural, that a canonical maximal representative exists up to equivalence under maximality conditions, and that meta-classification of $\LCls$ meta-structures stabilizes at the same $(\infty, \infty)$-theory (with its set-theoretic realization ascending the Grothendieck-universe hierarchy at each iteration).
...
\end{itemize}
We denote the class of such $\mathbf{F}$ as $\Meta_{\mathrm{Cls}}$.
\end{definition}
...
\end{itemize}
We denote the class of maximal meta-structures as $\Meta_{\mathrm{Cls}}^{\top}$.
\end{definition}
